...
- `llm.base_url`
  ...
  - 許可 `apply_scope`: `runtime`, `next_boot`
  - 制約: 0 文字以上 512 文字以下
  - 役割: 認知生成に使うプロバイダの Base URL を切り替える

- `llm.embedding_provider`
  - `value_type`: `string`
  - 許可 `apply_scope`: `runtime`, `next_boot`
  - 制約: 1 文字以上 64 文字以下
  - 役割: 埋め込み生成に使うプロバイダ識別子を切り替える
...
- `llm.embedding_base_url`
  ...
  - 許可 `apply_scope`: `runtime`, `next_boot`
  - 制約: 0 文字以上 512 文字以下
  - 役割: 埋め込み生成に使うプロバイダの Base URL を切り替える
...
- `output.mode`
  ...
  - 許可 `apply_scope`: `runtime`, `next_boot`
  - 制約: 1 文字以上 64 文字以下
  - 役割: UI のみか、UI と TTS の両方へ出すかを切り替える
...
- `integrations.line.channel_access_token`
  ...
  - 許可 `apply_scope`: `runtime`, `next_boot`
  - 制約: 1 文字以上 4096 文字以下
  - 役割: LINE 通知に使うチャネルアクセストークンを保持する

- `integrations.line.to_user_id`
  - `value_type`: `string`
  - 許可 `apply_scope`: `runtime`, `next_boot`
  - 制約: 1 文字以上 512 文字以下
  - 役割: LINE 通知の送信先ユーザー ID を保持する
