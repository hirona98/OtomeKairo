...
- スキーマ版が一致しない場合、暗黙の migration やフォールバック起動は行わない
...
- `schema_version` は、初期値 `1` の整数 JSON に固定する
...
3. `db_meta` の必須キーを upsert して、`schema_version=1` と `schema_name=core_schema` を確認する
4. 単一断面テーブルの seed 行を upsert する
5. ここまで成功したときだけ、後続の Web サーバまたはランタイム起動へ進む
...
3. lease 取得に成功したら、`owner_token` を保持したまま `短周期ループ` と `長周期ループ` の管理へ入る
4. 起動中は定期的に `heartbeat_at` と `expires_at` を更新する
5. 正常終了時は、保持中の `primary_runtime` lease を解放する
...
- `personality_json` は空オブジェクト `{}` に固定する
- `current_emotion_json` は `{"v":0.0,"a":0.0,"d":0.0,"labels":["neutral"]}` に固定する
- `long_term_goals_json` は空配列 `[]` に固定する
- `relationship_overview_json` は空オブジェクト `{}` に固定する
- `invariants_json` は空配列 `[]` に固定する
...
- `priority_effects_json` は空オブジェクト `{}` に固定する
...
- スキーマ版 `1` と `core_schema` が一致しない場合は起動しない
