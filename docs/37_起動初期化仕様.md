...
- `run_web.py` は、Uvicorn 標準の signal capture を使わず、`SIGINT` と `SIGTERM` を `server.should_exit` / `server.force_exit` へ変換して graceful shutdown する
- `run_web.py` は、Uvicorn のアクセスログを有効のまま使い、`/api/status` と `/api/chat/stream` だけをフィルタして定期ログを抑止する
- `run_web.py` は、通常 shutdown で発生する `KeyboardInterrupt` や `CancelledError` の想定内トレースを `uvicorn.error` から抑止してよい
- `run_runtime.py` は、`LiteLLM` の `DEBUG` ログを `log/otomekairo.log` に残し、端末には通常 `INFO` 以上だけを出してよい
- `run_runtime.py` は、`SIGINT` と `SIGTERM` を graceful shutdown に変換し、signal handler 自身ではログを書かず、`run_forever()` の `finally` で lease 解放まで必ず到達させる
...
- どちらかの子プロセスが終了した場合、親プロセスはまず `SIGINT` で両方の子プロセスを graceful shutdown させ、それでも残る場合だけ強制終了してから終了する
