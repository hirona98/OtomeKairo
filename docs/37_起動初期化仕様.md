...
- `schema_version` は、初期値 `4` の整数 JSON に固定する
...
3. 既存 DB が旧版なら、定義済みの one-step migration を順に適用して `schema_version=4` へ上げる
4. `db_meta` の必須キーを upsert して、`schema_version=4` と `schema_name=core_schema` を確認する
...
- `schema_version=2` から `3`、`schema_version=3` から `4` への migration だけを行ってよい
