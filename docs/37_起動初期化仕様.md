...
- `schema_version` は、初期値 `7` の整数 JSON に固定する
...
3. 既存 DB が旧版なら、定義済みの one-step migration を順に適用して `schema_version=7` へ上げる
4. `db_meta` の必須キーを upsert して、`schema_version=7` と `schema_name=core_schema` を確認する
...
- `schema_version=2` から `3`、`schema_version=3` から `4`、`schema_version=4` から `5`、`schema_version=5` から `6`、`schema_version=6` から `7` への migration だけを行ってよい
...
- スキーマ版 `7` と `core_schema` が一致しない場合は起動しない
