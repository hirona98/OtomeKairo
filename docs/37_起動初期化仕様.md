...
- `schema_version` は、初期値 `3` の整数 JSON に固定する
...
3. 既存 DB が `schema_version=2` なら、定義済みの one-step migration を適用して `schema_version=3` へ上げる
4. `db_meta` の必須キーを upsert して、`schema_version=3` と `schema_name=core_schema` を確認する
...
- `schema_version=2` から `3` への migration 以外は行わない
...
- `row_id=1` を upsert する
- `values_json` は、`docs/39_設定キー運用仕様.md` にある全設定キーを既定値で埋めた完全オブジェクトに固定する
...
- スキーマ版 `3` と `core_schema` が一致しない場合は起動しない
