...
- `run_all.py`、`run_web.py`、`run_runtime.py` は、それぞれ `launcher / web / runtime` 用の共通ロガーを初期化し、`log/otomekairo-*.jsonl` に `DEBUG` の JSONL ログを出し、端末には `INFO` 以上だけを出す
...
- `run_runtime.py` は、`LiteLLM` の `DEBUG` ログを `log/otomekairo-runtime.jsonl` に残し、端末には通常 `INFO` 以上だけを出してよい
