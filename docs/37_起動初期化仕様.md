...
- `run_all.py`、`run_web.py`、`run_runtime.py` は、それぞれ `launcher / web / runtime` 用の共通ロガーを初期化し、`log/otomekairo.log` に `DEBUG` の通常テキストログをまとめて出し、端末には `INFO` 以上を出しつつ、単体の JSON だけでなく複数行メッセージ内に埋め込まれた JSON や `context` の辞書も見やすく整形して表示する
