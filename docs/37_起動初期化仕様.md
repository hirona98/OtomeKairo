...
- `schema_version` は、初期値 `2` の整数 JSON に固定する
...
3. `db_meta` の必須キーを upsert して、`schema_version=2` と `schema_name=core_schema` を確認する
...
- 旧スキーマ版 `1` の DB を見つけても、暗黙 migration は行わず起動を失敗にする
...
- スキーマ版 `2` と `core_schema` が一致しない場合は起動しない
