...
- `schema_version` は、初期値 `11` の整数 JSON に固定する
...
3. 既存 DB が旧版なら、定義済みの one-step migration を順に適用して `schema_version=11` へ上げる
4. `db_meta` の必須キーを upsert して、`schema_version=11` と `schema_name=core_schema` を確認する
...
- `schema_version=2` から `3`、`3` から `4`、`4` から `5`、`5` から `6`、`6` から `7`、`7` から `8`、`8` から `9`、`9` から `10`、`10` から `11` への migration を行う
...
- `active_camera_connection_id` は `NULL` で初期化する
