...
- `self_state.personality` は、trait 値だけでなく、`preferred_interaction_style` と `habit_biases` を持てる形にする
...
- `personality_json` のトップレベルキーは、`trait_values`、`preferred_interaction_style`、`habit_biases` に固定する
...
- current 実装では、個別の好悪は `personality_json` に昇格させず、`preference_memory` を正本として扱う
...
- `attention_set` は、`personality`、`relationship_overview`、反復した回避傾向を評価軸として使う
