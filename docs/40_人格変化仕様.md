...
- 初期実装では、`attention_set` と `self_initiated` への反映は、`build_cognition_input` が作る `attention_score_breakdown` と `self_initiated_score_breakdown` を通じて短周期入力へ渡してよい
- 初期実装では、`skill promoter` は永続 `skill_registry` の代わりに、`learned_preferences` と `habit_biases` から人格に合う `skill_candidates` を作り、`avoided_action_styles` に触れる候補を落としてよい
