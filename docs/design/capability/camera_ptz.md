...
camera source は OtomeKairo の視覚であり、ユーザー明示指示、API起床、定期思考、capability result follow-up、自律判断のいずれでも利用できる。
...
採用した camera source は `camera_source.enabled=true` のとき定期思考処理の観測対象に含める。
...
- API起床または定期思考で、現在の camera 視覚前景だけでは判断対象が見切れている、低信頼、または明らかに視野外である
