...
API 仕様ファミリー全体の入口と分割方針は [README.md](README.md) を正とする。
この文書では、その配下にある詳細文書へ共通して適用するルールだけを扱う。

API を変更するときは、該当する詳細文書に加えて [README.md](README.md) の見取り図と、この文書の共通ルールがずれていないかを同じ変更内で確認する。
この API 仕様ファミリーは、HTTP / WebSocket API 仕様を含む。
...
ただし `GET /api/events/stream`、`GET /api/logs/stream`、`GET /api/audio/stream`、`GET /ui/api/events/stream`、`GET /ui/api/logs/stream`、`GET /ui/api/audio/stream` は WebSocket upgrade endpoint として扱い、この共通 envelope から外す。
...
上の 3 つは未認証で受け付ける。
`POST /api/bootstrap/acquire-console-access-token` は未発行時に token を発行し、発行済み状態では既存 token を返す。
