...
- `selected_model_preset` を詳細付きで返す場合も、`roles.*.api_key` の実値は返さない
- 通常の読み取り系では、秘密値の有無を `api_key_present` で返してよい
- ただし `editor-state` は編集 bundle なので、`model_presets` には `roles.*.api_key` を含めてよい
...
- `editor-state.model_presets` は編集対象なので、`roles.*.api_key` の実値を含む shape で扱う
...
| `404` | `source_memory_set_not_found` | 複製元の記憶集合が存在しない |
| `404` | `model_preset_not_found` | 指定したプリセットが存在しない |
| `409` | `memory_set_id_already_exists` | 複製先の `memory_set_id` が既に存在する |
