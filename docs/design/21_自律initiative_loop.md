...
`suppression_summary` は `suppression_level / reason_summary / foreground_override / desktop_reply_priority_kind / cooldown_policy` を持ちうる。
...
その場限りの「控える」「見守る」は、未知または大きく変化した desktop observation より優先しない。
...
`background_wake` の観測中に `conversation_input` または `reply` が新しく増えた場合、server は観測前の直近会話 snapshot を使って発話せず、`noop` にする。
desktop capture の取得結果は画像意味理解へ通し、詳細な視覚説明を `visual_observation_record` として保存する。
desktop capture の短い scene signature と直近自発 reply 済み scene は novelty 判定用の process-local runtime に保持する。
`world_state` には詳細な視覚説明そのものではなく、現在判断に効く短い `visual_context.summary_text` を反映する。
desktop capture の scene signature は `vision_source_id / source_label / active_app / visual_summary_text` を持ち、`window_title` を持たない。
desktop capture の scene signature 比較は `vision_source_id / active_app` の不一致と `visual_summary_text` の類似度を使い、`window_title` を使わない。
...
`changed` の desktop novelty は `change_strength=normal / significant` を持つ。
`vision_source_id` または `active_app` が異なる場合、または `visual_summary_text` の類似度が `0.22` 未満の場合、`change_strength=significant` とする。
desktop capture は `interrupt_worthiness=low / medium / high` を持つ。
`novelty_kind` は観測の新しさであり、`interrupt_worthiness` は自発 reply として割り込む価値である。
`changed` かつ `change_strength=significant` は `interrupt_worthiness=medium` とする。
`first_success` と `changed` かつ `change_strength=normal` は `interrupt_worthiness=low` とする。
`pending_after_cooldown` は `interrupt_worthiness=medium` とする。
`first_success / changed / pending_after_cooldown` の desktop novelty は wake 判断へ進む前景シグナルとして扱う。
`desktop_observation_signal.reply_eligibility=eligible` は、`interrupt_worthiness` が medium 以上の desktop scene を根拠に短い自発 reply の候補へ進めることを表す。
...
ただし、`desktop_observation_signal.reply_eligibility=eligible` かつ `novelty_kind=pending_after_cooldown`、または `novelty_kind=changed` かつ `change_strength=significant` の場合、initiative loop は `desktop_reply_priority` を立てる。
`desktop_reply_priority` がある場合、`background_wake`、cooldown、`pending_intent` 不在、capability 不在、`initiative_baseline=low` は単独の抑制理由にしない。
この場合、`suppression_summary.foreground_override=desktop_reply_priority` とし、cooldown 中なら `cooldown_policy=soft` を付ける。
同じ `dedupe_key` の直近介入と `ongoing_action.status=waiting_result` は引き続き抑制理由として扱う。
...
background wake でも `desktop_observation_signal.reply_eligibility=eligible` かつ `preferred_result_kind=reply` の場合、画面観測に根拠づく短い reply を `noop` より優先する。
`cooldown_active=true` でも、`changed` かつ `change_strength=significant`、または `pending_after_cooldown` は見慣れない前景として短い reply を優先する。
この場合、`suppression_summary.foreground_override=desktop_reply_priority` を付け、`background_wake`、cooldown、`pending_intent_summaries` が空であることを単独の `noop` 理由にしない。
`cooldown_active=true` の `first_success` や通常の `changed` は過剰介入を避けるため `noop` を許可し、reply にならなかった場合は `pending_novel_scene` として保持する。
...
`suppression_summary.foreground_override=desktop_reply_priority` の場合、LLM は短い reply を優先し、`noop` を選ぶなら画面内容に基づく cooldown 以外の具体的な見送り理由を `reason_summary` に書く。
decision contract validation は、initiative の selected candidate entry が `preferred_result_kind=capability_request` の場合に `capability_request` 以外を repair 対象にし、`preferred_capability_id` と異なる capability request も repair 対象にする。`preferred_result_kind=capability_request` ではない場合の `capability_request`、`fresh_world_state_available=true` の capability request、同じ `vision_source_id` の新鮮な `vision.capture` request は repair 対象にする。`desktop_observation_signal.reply_eligibility=eligible` かつ `pending_after_cooldown`、cooldown 中ではない `first_success`、`response_target=user`、due の `pending_intent`、継続中の `ongoing_action`、または強い `drive_state` と grounded foreground がある場合、ready / grounded foreground かつ `preferred_result_kind=reply` の `noop` を repair 対象にする。
cooldown 中の `changed` + `change_strength=significant` は prompt と candidate reason で短い reply を優先するが、decision contract validation では `noop` を不正扱いしない。
