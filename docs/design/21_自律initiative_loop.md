...
`candidate_families` の各 entry は `family / available / selected / priority_score` を含む。
...
自発系の成立条件は、`drive_state` と現在文脈の噛み合いである。
`pending_intent` が空の場合も、通常の判断入力へ進める。
...
起床前観測 の運用時刻は `wake_policy` と process-local runtime で扱い、成功結果はその回の initiative 判断へ進む前景シグナルとして扱う。
...
LLM は `change_state`、`response_target`、候補理由、抑制要約を合わせて `speech / noop / pending_intent` を選ぶ。
...
強い `drive_state` が特定の status family を要求する場合は、対応 state type の鮮度に応じて status capability、既存要約、または `vision.capture` のいずれかを選ぶ。
