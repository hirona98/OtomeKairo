...
background wake でも `desktop_observation_signal.reply_eligibility=eligible` かつ `preferred_result_kind=reply` の場合、画面観測に根拠づく短い reply を `noop` より優先する。ただし `cooldown_active=true` の `first_success / changed` は過剰介入を避けるため `noop` を許可し、reply にならなかった場合は `pending_novel_scene` として保持する。
