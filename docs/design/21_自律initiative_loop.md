...
`suppression_summary` は `suppression_level / reason_summary / background_trigger / cooldown_active / same_dedupe_recently_replied` を持ちうる。
...
visual capture の短い observation signature と直近自発 reply 済み signature は変化判定用の process-local runtime に保持する。
...
visual capture の変化は `first_seen / changed / stable / same_as_recent_reply` の `change_state` に正規化する。
`first_seen / changed` は wake 判断へ進む前景シグナルとして扱うが、ユーザーへの reply 必要性を直接表さない。
`same_as_recent_reply / stable` は繰り返し発話を避ける材料として扱う。
`current_input.sender=system` かつ `current_input.response_target=none` の `wake / background_wake` では、visual observation をユーザー発話やユーザーへの応答要求として扱わず、reply 本文は観測に根拠づける。
...
cooldown 中の `first_seen / changed` でも reply 義務にはせず、LLM が `reply / noop / pending_intent` を選ぶ。
...
ただし、視覚観測の変化だけで cooldown、`background_wake`、`initiative_baseline=low` を上書きする reply priority は立てない。
同じ `dedupe_key` の直近介入、cooldown、`ongoing_action.status=waiting_result` は引き続き抑制理由として扱う。
...
background wake でも `visual_observations[].change_state=first_seen / changed` は判断材料にするが、それだけで `reply` を固定しない。
`cooldown_active=true` の視覚変化は過剰介入を避けるため `noop` を許可する。
`current_input.sender=system` かつ `current_input.response_target=none` の background wake では、visual observation を「ユーザーへの応答要求」として扱わず、ユーザー発話への相づちとして reply を始めない。
