...
記憶領域の上位意味は [人格と記憶.md](人格と記憶.md)、詳細契約は [../memory/README.md](../memory/README.md) を正とする。
...
capability manifest、binding、state の詳細契約は [../capability/capability_manifest.md](../capability/capability_manifest.md) を正とする。
...
1. 対話入力、API起床要求、観測結果、実行結果を受け、必要なら身体や外界を観測する
...
flowchart TD
    ...
    Wait([待機])
    Observe[対話入力、API起床要求、観測結果、実行結果を受け、必要なら観測する]
    Build[現在の文脈を解釈する]
...
外部接続 connector の repository 配置と本体 package との依存境界は [../integration/外部接続connector配置方針.md](../integration/外部接続connector配置方針.md) を正とする。
...
具体的な接続方式と権限境界の詳細は [../integration/接続と権限境界.md](../integration/接続と権限境界.md) を正とする。
...
- `current_input`
  - その作業が答え続ける向き。対話、感覚入力、外界イベント、実行結果などを含む。到着との分離は [../llm/プロンプト文脈分離方針.md](../llm/プロンプト文脈分離方針.md) を正とする
- `world_state`
...
モデルプリセットの詳細な切り方と設定管理は [../configuration/モデルプリセット詳細.md](../configuration/モデルプリセット詳細.md) を正とする。

## モデル役割の論理分割

生成系 role の一覧と各 role の責務は [../configuration/モデルプリセット詳細.md](../configuration/モデルプリセット詳細.md) を正とする。
この文書では、role を用途ごとに論理分割し、モデル実体とは別に扱う方針だけを定める。
この役割分割は論理上のものであり、別々のモデル実体を要求しない。
OtomeKairo では、全生成処理で共有する設定を `model_preset` としてまとめ、`selected_model_preset_id` で選択する。
一方で、埋め込みモデルは生成系 role とは別に `memory_set` に属する設定として扱う。

自律判断では、`wake_policy` が与えた判断機会から `initiative_context` を組み立て、通常の `decision_generation` に渡す。
自律 initiative loop の責務境界は [../runtime/自律initiative_loop.md](../runtime/自律initiative_loop.md) を正とする。
