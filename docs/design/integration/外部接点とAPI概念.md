...
外部接点は対話入力、観測結果、API起床要求、実行結果、変更要求を送り、結果や状態を受け取るが、判断や設定の正本にはならない。

この文書では、path や wire 形式ではなく、外向きにどんな面を持つかを定義する。
具体的な API 仕様は [../api/README.md](../api/README.md) と `docs/design/api/` 配下を正とする。
...
選択中モデルプリセットの `web_search_enabled` は、生成 provider へ渡す検索利用指示であり、実行連携面の capability ではない。
...
外部接続 connector の repository 配置と依存境界は [外部接続connector配置方針.md](外部接続connector配置方針.md) を正とする。
...
- アバター音声プリセットとマイク設定の取得
- 設定定義の作成、複製、差し替え、削除

変更単位の意味境界は [../configuration/設定変更.md](../configuration/設定変更.md) を正とする。
記憶集合は選択、空集合作成、集合単位の複製、集合単位の削除の対象である。
個別の記憶行に対する確認、承認、訂正、撤回、削除、初期化、export は設定面へ入れない。
...
`GET /api/catalog` は、人格設定、記憶集合、モデルプリセットだけを返す。
capability manifest、binding、availability の詳細は [../capability/capability_manifest.md](../capability/capability_manifest.md) を正とする。
