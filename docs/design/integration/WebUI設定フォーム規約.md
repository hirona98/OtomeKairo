...
- 接続クライアント ID などシステム固定値は input を `disabled` で表示する（カメラ・MCP と同じ）

## 定期思考の単発実行

定期思考タブの「単発実行」は、保存済み設定を使う独立した操作として表示する。押下時からブラウザで3秒を数え、経過後に [単発定期思考 API](../api/bootstrapと入力.md#webui-からの単発定期思考) へ1回だけ要求する。待機中と要求処理中はボタンを無効化し、待機中は残り秒数、終了時は結果または失敗を表示する。設定パネルを閉じても、ページが開いているあいだは開始済みのカウントを続ける。
