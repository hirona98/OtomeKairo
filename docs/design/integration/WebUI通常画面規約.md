...
- マイク操作行の直下に音声観測行（`.audio-meters`）を置く
  - 入力レベル（`vad.dbfs`）
  - VAD しきい値超過（`vad.probability` と threshold の比較。probability 未取得時は 0.0 として表示）
  - 識別しきい値超過（新しい `last_utterance_result.utterance_seq` を受信してから3秒間だけ `top1_similarity` / `threshold_met` を表示し、その後は 0.0 に戻す）
  ...
  - 右端のバッジは置かず、数値列は固定幅として桁変化でバー（track）幅を変えない
...
初期表示では `GET /api/conversation/history` から現在の `interaction_ref` に属する直近30件を古い順に表示し、取得完了後に event stream の購読を開始する。保存済み画像は存在しないため履歴表示には画像を含めない。過去イベントに表示名が保存されていない場合は空の表示名として扱い、別の現在設定値で補完しない。
