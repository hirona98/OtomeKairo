...
- 複数候補が並んだとき、現在入力との噛み合いより時刻順が優先される
- `wake` では、最近 reply 済みで抑制される候補が先頭に来ると、他候補があっても起床機会を無駄にしやすい

この方式は初期段階では壊れにくかったが、本命実装としては次の問題が残る。
...
5. trigger ごとの流れに応じて、入力文へ差し込むか、今回は差し込まないかを決める
...
```json
{
  ...
  "selection_reason": "今の入力だけでは、どの保留候補を前に出しても自然さが弱い。"
}
```
...
3. 候補が選ばれたときだけ、その候補要約を `desktop_watch` 入力文へ足す
4. `selected_candidate_ref=none` でも、`desktop_watch` 入力自体は通常どおり共有判断パイプラインへ流す
...
`docs/design/21_自律initiative_loop.md` は、保留意図が無い場合も含めた自律判断機会の組み立てを定める。
