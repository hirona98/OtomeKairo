...
autonomous speech の入口として扱う `drive_state` は、`support_count / support_strength / signal_strength / freshness_hint / stability_hint` の構造値で speech-ready と判定されたものに限る。
...
`suppression_summary` は `suppression_level / background_trigger / same_dedupe_recently_replied / recent_spontaneous_speech` を含む。
`suppression_summary.seconds_since_last_spontaneous_speech` は直近の定期起床自発発話からの経過秒数である。
`suppression_summary.suppression_window_minutes` は直近自発発話による抑制窓の分数である。
...
- 自発系
  - speech-ready な `drive_state`、または強い `entry_basis` を持つ `initiative_entry_summary.entry_kind=enter` と現在文脈が噛み合い、前へ出る理由があるもの

自発系の成立条件は、speech-ready な `drive_state`、`ongoing_action`、`pending_intent`、または強い `entry_basis` を持つ `initiative_entry_summary` と現在文脈の噛み合いである。
...
`pending_intent` が空の場合も、speech-ready な `drive_state`、`autonomous_run`、`ongoing_action`、または強い `entry_basis` を持つ `initiative_entry_summary.entry_kind=enter` があれば通常の判断入力へ進める。
...
優先順位は `user_message > capability result handling > due autonomous_run > background_wake` にする。
...
speech-ready な `drive_state`、`pending_intent`、進行中 action、強い `entry_basis` を持つ `initiative_entry_summary.entry_kind=enter` が無い場合、視覚変化だけでは通常の判断入力へ進まない。
...
autonomous family の availability は、強い `initiative_entry_summary` または speech-ready な `drive_state` で決める。
autonomous family の priority は、speech-ready な `drive_state`、現在文脈、前景世界状態、候補理由の強さで決める。
...
| `background-grounded-speech` | interval 初回起床で予定 `world_state` と整合する speech-ready な `drive_state` がある | `wake_scheduler_active=true`、`foreground_thinness=grounded`、`selected_candidate_family=autonomous`、`decision.kind=speech`、`memory_trace=succeeded` |
...
speech-ready な `drive_state` があり、対応する grounded foreground がない場合、追加観測を短い speech より優先する。
speech-ready な `drive_state` が特定の status family を要求する場合は、対応 state type の鮮度に応じて既存要約または capability を選ぶ。
