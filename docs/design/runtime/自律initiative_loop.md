...
`background_wake` は定期起床による自己評価である。
...
`background_wake` は発話自然度を 10 段階で内的に見積もり、5 を標準基準にする。
5 は、短い独話として成立し、明確な反復、観測不足、明示境界、プライバシー境界が上回らない状態である。
評価値は JSON や `reason_summary` に出力しない。
`first_seen / changed` は、具体的な前景がある場合に 5 近辺の `speech` 候補として扱う。
...
`wake / background_wake` の「定期起床」「wake」という入力文言は、定期起床による自己評価の説明として扱う。
`wake / background_wake` の入力文は、観測、`drive_state`、直近文脈、候補を合わせて、関わる、保留する、見送る、能力を使うのどれが自然かを評価する自律判断機会を表す。
...
起床前観測 の運用時刻は `wake_policy` と process-local runtime で扱い、成功結果は内部観測と自律判断の材料として扱う。
...
`current_input.sender=system` かつ `current_input.response_target=none` の `wake / background_wake` では、decision は観測、候補、現在文脈を比較して `speech / noop / pending_intent / capability_request` を選ぶ。
...
| `background-weak-foreground-noop` | interval 初回起床で `visual_context` 系の薄い `world_state` だけがあり、視覚観測の `change_state` が `stable / same_as_recent_speech` である | `initiative_context` なしの `decision.kind=noop`、`memory_trace=skipped` |
| `background-grounded-speech` | interval 初回起床で予定 `world_state` と整合する構造値が強い `drive_state` がある | `wake_scheduler_active=true`、`foreground_thinness=grounded`、`selected_candidate_family=autonomous`、`decision.kind=speech`、`memory_trace=succeeded` |
