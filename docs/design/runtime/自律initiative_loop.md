...
- 自発系
  - 強く前景化した `drive_state`、または強い `entry_basis` を持つ `initiative_entry_summary.entry_kind=enter` と現在文脈が噛み合うもの

自発系は、強く前景化した `drive_state`、`ongoing_action`、`pending_intent`、または強い `entry_basis` を持つ `initiative_entry_summary` と現在文脈の噛み合いを材料にする。
`visual_context`、視覚観測の `first_seen / changed`、直近会話は、他の材料と同じ盤面で比較する前景シグナルである。
...
`pending_intent` が空の場合も、`drive_state`、`autonomous_run`、`ongoing_action`、または強い `entry_basis` を持つ `initiative_entry_summary.entry_kind=enter` があれば通常の判断入力へ進める。
...
`wake / background_wake` の「定期起床」「wake」という入力文言は、定期的な自己評価起点の説明として扱う。
...
autonomous family の availability は、強い `initiative_entry_summary` または構造値が強い `drive_state` を中心に組み立てる。
