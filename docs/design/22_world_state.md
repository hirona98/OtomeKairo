...
`vision.capture` の `source_kind=desktop` はその瞬間だけ見るキャプチャなので、`visual_context` に追加しない。
...
`source_kind=desktop` の `vision.capture` は `VisualObservationContext` として判断サイクルだけに渡し、`world_state` へ保存しない。
...
ただし desktop capture の `visual_summary_text` は `world_state` source pack の `visual_context` に渡さない。
...
- `source_kind=desktop` の `vision.capture` result は `visual_context` を作らない
