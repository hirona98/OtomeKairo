...
視覚前景の補助要約がある場合は `visual_context` を追加する。
...
- `visual_context / body / device` は state_type ごとの foreground slot 単位で置換する
...
- `world_state_trace.source_pack_state_type_hooks` として、`visual_context / social_context / environment / location / external_service / body / device / schedule` ごとの `summary_text / summary_source / signal_fields / capability_id` 要約
