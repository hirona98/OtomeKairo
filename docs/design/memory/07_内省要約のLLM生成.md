...
- 将来 `predicate` や `qualifiers` が増えても、文面生成コードを増やし続けなくて済むようにする
...
現在の background job は embedding 同期を前提にした最小の state snapshot しか持たない。
