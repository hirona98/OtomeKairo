...
```json
{
  ...
  "support_kinds": ["episodes", "memory_units", "persona", "affect_state"],
  "evidence_counts": {
    "episodes": 3,
    "memory_units": 4,
    "support_cycles": 3,
...
- `relationship / self` では、selected persona の短い snapshot を補助入力として渡す
- `self` では `mood_state`、`relationship / user` では対応する `affect_state` を補助入力として渡してよい
...
- `persona` は言い回しと注目点の補助に留め、`episodes / memory_units` の外側を上書きしない
- `mood_state` と `affect_state` は、`episodes / memory_units` と整合する範囲だけで補助的に使う
- `open_loops` は長期傾向に効くときだけ触れる
- 単発イベントの説明ではなく、継続パターンとして要約する

user prompt では、scope 情報、counts、既存 summary、persona / mood / affect の補助情報、episode 要点、memory_unit 要点をそのまま構造化で渡す。
