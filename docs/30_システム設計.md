...
- `src/otomekairo/infra/sqlite/backend.py` の残存責務は bootstrap、schema/seed、共通 connection、`write_memory` の execution state load / apply helper であり、現在の主要な残存結合点は `write_memory` 更新ロジックの private helper 集約である
...
- current 実装では、`write_memory` の手順制御は `usecase/run_write_memory_job.py` が担当し、transaction 境界は `gateway/unit_of_work.py`、`infra/sqlite/unit_of_work.py`、`infra/sqlite/write_memory_execution_store.py` に明示し、`src/otomekairo/infra/sqlite/backend.py` は execution state load と SQL 適用を担う
...
3. `src/otomekairo/infra/sqlite/backend.py` の内部実装を、SQLite を共有する複数 helper module / adapter へ分割し、具象依存を `boot/` へ押し戻す
...
6. `src/otomekairo/infra/sqlite/backend.py` を bootstrap + `write_memory` apply までへ縮退し、残る helper 分割と関連 docs 同期を進める
