...
- Web サーバの公開面は `status api`、`settings api`、`settings editor api`、`chat input api`、`chat stream api`、`camera api`、`/` の静的 UI である
- ランタイムの主ループは `settings_change_sets`、`settings_overrides`、`pending_inputs`、`waiting_external` の `browse` task、`memory_jobs` を順に消費する
- current の入力種別は `chat_message`、`camera_observation`、`network_result`、`cancel` に限る
- current の行動確定は `speak`、`browse`、`notify`、`look`、`wait` に限り、`move` と `social` は target の責務名だけ先に存在する
- current の外部接続は TTS、DuckDuckGo 検索、ONVIF カメラ静止画取得と視点操作であり、`microphone`、`SNS`、移動系アダプタは未接続である
