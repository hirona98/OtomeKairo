...
6. 長周期 helper は `write_memory_execution_store.py` とその下位 helper module 群へ集約し、`write_memory_state_update_impl.py` / `write_memory_self_state_sync_impl.py` / `write_memory_event_affect_impl.py` / `write_memory_context_relation_impl.py` まで分けた
