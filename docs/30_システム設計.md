...
- current の composition root は `src/otomekairo/boot/compose_sqlite.py`、`src/otomekairo/boot/compose_runtime.py`、`src/otomekairo/boot/compose_web.py` にあり、`SqliteStateStore` と SQLite adapter 群の組み立ては `boot/` で完結する
- current の `state committer` は汎用抽象ではなく、`sqlite_state_store.py` の `finalize_pending_input_cycle`、`finalize_task_cycle`、`finalize_settings_override`、`finalize_settings_change_set` を中心に状態差分を確定する
...
- `src/otomekairo/web/app.py` と `src/otomekairo/web/dependencies.py` は `AppServices` bundle と `gateway/*` port だけを扱い、`SqliteStateStore` を直接生成しない
...
- `src/otomekairo/boot/run_all.py` と deterministic eval runner は `src/otomekairo/boot/compose_sqlite.py` を経由して port / adapter bundle を組み立て、`SqliteStateStore` を直接生成しない
- `src/otomekairo/infra/sqlite/unit_of_work.py` と `src/otomekairo/infra/sqlite/memory_job_store.py` が `write_memory` と `embedding_sync` の completion transaction を担当し、`sqlite_state_store.py` の公開 API から長周期 orchestration を追い出している
- `src/otomekairo/infra/sqlite_state_store.py` は依然として bootstrap、runtime 読み出し、短周期 commit、長周期 `memory_jobs`、設定 editor 読み書き、followup enqueue を 1 クラスへ抱えており、現在の主要な残存結合点である
- `src/otomekairo/infra/sqlite/*.py` の adapter 群は責務ごとに分かれたが、内部では同じ SQLite backend を共有している
- deterministic eval 用 usecase は `SqliteStateStore` へ直接依存せず、boot 側で組み立てた store bundle と callable を受け取る
...
- current 実装では、`write_memory` の手順制御は `usecase/run_write_memory_job.py` が担当し、transaction 境界は `gateway/unit_of_work.py` と `infra/sqlite/unit_of_work.py` に明示し、`sqlite_state_store.py` は同一 transaction 内の read/write helper と SQL 適用を担う
...
1. `docs/10` と `docs/30` を正本として、port 名、責務、composition root の位置を先に固定する
2. `gateway/` に `runtime query`、`cycle commit`、`memory jobs`、`settings editor`、`ui events`、`runtime lease`、`unit_of_work` の抽象を追加する
3. `sqlite_state_store.py` の内部実装を、SQLite を共有する複数 adapter へ分割し、具象依存を `boot/` へ押し戻す
4. `runtime/` と `web/` を dependency bundle 経由へ切り替え、具象 method 名への依存を外す
5. `write_memory` を `unit_of_work` 境界へ載せ替え、usecase から SQLite helper 前提を消す
6. `SqliteStateStore` 相当の巨大 adapter を削除し、関連 docs を current 実装に同期する
