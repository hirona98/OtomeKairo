...
- current の行動確定は `speak`、`browse`、`notify`、`look`、`wait` に限り、`move` と `social` は target の責務名だけ先に存在する
- current の外部接続は TTS、DuckDuckGo 検索、ONVIF カメラ静止画取得と視点操作、ブラウザ録音を `AmiVoice` へ流して `microphone_message` を作る `STT` 経路であり、`microphone` の live sensor、`SNS`、移動系アダプタは未接続である
...
- `外部インタフェース群` は、カメラ、マイク、TTS、SNS API、ブラウザアクセスなどの外界接続を担う
...
- current 実装では、`write_memory` の手順制御は `usecase/run_write_memory_job.py` が担当し、`sqlite_state_store.py` は同一 transaction 内の read/write helper と SQL 適用だけを担う
...
- `retrieval selector` は、preview と要約済み候補だけを `LLM` へ渡し、`selected`、`reserve`、`slot skipped` を固定 shape で返す
- `context assembler` は、`memory_bundle` を直接 prompt へ流さず、`recent_dialog`、`selected_memory_pack`、`confirmed_preferences`、`long_mood_state`、`reply_render_input`、`action_selection_context` へ再投影する
- `reply renderer` は full `cognition_input` を見ず、`reply_render_input` と `reply_render_plan` だけを入力にする
- `reply_render_input` には `confirmed_preferences` と `revoked_preferences` の両方を入れ、負の嗜好断面を行動判断と共有する
- `retrieval_runs` は、collector 件数、selector 入力件数、`selection_trace`、review 用 detail を持ち、`retrieval_eval` と `retrieval_triage` の正本入力になる
