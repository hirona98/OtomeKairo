...
- current の `state committer` は汎用抽象ではなく、`src/otomekairo/infra/sqlite/backend.py` の `finalize_pending_input_cycle`、`finalize_task_cycle`、`finalize_settings_override`、`finalize_settings_change_set` を中心に状態差分を確定する
...
- `src/otomekairo/infra/sqlite/backend.py` は依然として bootstrap、runtime 読み出し、短周期 commit、長周期 `memory_jobs`、設定 editor 読み書き、followup enqueue を 1 クラスへ抱えており、現在の主要な残存結合点である
