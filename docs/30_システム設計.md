...
- current の `state committer` は汎用抽象ではなく、`src/otomekairo/infra/sqlite/cycle_commit_impl.py` と `src/otomekairo/infra/sqlite/settings_impl.py` が短周期確定を担当し、`src/otomekairo/infra/sqlite/backend.py` は内部 helper と transaction 内 SQL 適用を支える
...
- `src/otomekairo/infra/sqlite/runtime_query_impl.py`、`cycle_commit_impl.py`、`settings_impl.py`、`ui_event_impl.py`、`runtime_lease_impl.py`、`memory_job_impl.py` が責務別の SQLite 実装を持ち、`*_store.py` はその薄い adapter として gateway port を満たす
- `src/otomekairo/infra/sqlite/unit_of_work.py` と `src/otomekairo/infra/sqlite/memory_job_store.py` が `write_memory` と `embedding_sync` の completion transaction を担当し、`src/otomekairo/infra/sqlite/backend.py` の公開 API から長周期 orchestration を追い出している
- `src/otomekairo/infra/sqlite/backend.py` の残存責務は bootstrap、共通 connection、runtime live state helper、event / memory job / write_memory apply helper であり、現在の主要な残存結合点はこの private helper 集約である
...
- current 実装では、`write_memory` の手順制御は `usecase/run_write_memory_job.py` が担当し、transaction 境界は `gateway/unit_of_work.py` と `infra/sqlite/unit_of_work.py` に明示し、`src/otomekairo/infra/sqlite/backend.py` は同一 transaction 内の read/write helper と SQL 適用を担う
...
3. `src/otomekairo/infra/sqlite/backend.py` の内部実装を、SQLite を共有する複数 adapter へ分割し、具象依存を `boot/` へ押し戻す
...
6. `SqliteBackend` 相当の巨大 backend を削除し、関連 docs を current 実装に同期する
