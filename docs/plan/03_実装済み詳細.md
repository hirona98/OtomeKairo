...
この文書は、現行計画から切り離した「実装済み詳細」の参照先である。
計画判断や優先順位の更新は `docs/plan/01_現行計画.md` を正とし、ここでは現在の実装が依存している前提、コード構成、完了済み段階の詳細だけを残す。
...
- `src/otomekairo/store.py`
  - `SQLiteMemoryStore`
  - SQLite 永続化の facade
  - cycle / memory / inspection query
- `src/otomekairo/store_schema.py`
  - SQLite schema 初期化
- `src/otomekairo/store_vector.py`
  - vector index 永続化
  - vector 類似検索
- `src/otomekairo/store_clone.py`
  - memory set clone / delete
- `src/otomekairo/service.py`
  - service 依存関係の組み立て
  - background scheduler の起動 / 停止
- `src/otomekairo/service_config.py`
  - bootstrap / config / catalog / inspection API
...
- `src/otomekairo/service_memory.py`
  ...
  - memory trace 更新
  - reflective / vector failure の監査補助
- `src/otomekairo/service_spontaneous.py`
  - `wake` / `desktop_watch`
  - pending_intent 選別と runtime queue
...
- `src/otomekairo/recall.py`
  - `RecallBuilder` の高水準 orchestration
  - 構造レーン
  - scope context 構築
  - `RecallPack` 組み立て
...
- `event_evidence`
  ...
  - `event_evidence_generation` role で `anchor / topic / decision_or_result / tone_or_note` を LLM 圧縮する
  - 生成失敗は event 単位で閉じ込め、`retrieval_runs` と `recall_trace` に残す
- `RecallPack` の意味的選別
  - `recall_pack_selection` role に候補 section 群と conflict source pack を渡す
  - LLM が `section_selection` と `conflict_summaries` を返し、コード側で global limit / dedupe / section limit を強制する
  - 失敗は `recall_pack_selection_failure` event と `recall_trace.recall_pack_selection` に残す
- `RecallPack` 本接続
  - `decision_generation` と `expression_generation` へ internal context として渡す
  - `retrieval_runs` と trace に selection 要約を残す
- `reflective consolidation`
  - `summary` 生成
...
- 保留意図
  ...
  - `pending_intent_selection` role に eligible candidate 群を渡し、`selected_candidate_ref | none` を返させる
  - 選別結果と失敗を `observation_trace.pending_intent_selection` と `pending_intent_selection_failure` event に残せる
...
- wake observation
  ...
  - eligible な保留意図候補群から LLM が今の再評価対象 1 件または `none` を選ぶ
  - `none` のときは wake 専用 `noop` として閉じる
  - `reply` なら候補を消費する
...
- desktop_watch
  ...
  - eligible な保留意図候補群から LLM が今の再評価対象 1 件または `none` を選び、`none` でも通常観測は継続する
...
- eligible な保留意図候補群から LLM が今の再評価対象 1 件または `none` を選ぶ
- `reply` なら候補を消費し、`none` なら wake 専用 `noop` で閉じる
...
- `desktop_watch` 開始時に eligible な pending-intent 候補群を LLM 選別し、候補が無くても通常観測を継続できる
- `reply` になった場合は `desktop_watch` event を stream へ返せる

この段階でも、画像内容の意味理解まではまだやらない。
現行段階では capture image を通知添付に使い、判断は `client_context` 主体に留める。
