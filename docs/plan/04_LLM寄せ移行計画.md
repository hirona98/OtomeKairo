...
次にやるべき実装は Phase 5 である。

理由は次のとおりである。

- Phase 4 で自発再介入の候補選択まで LLM 寄せが通り、次は `desktop_watch` 側の観測意味理解が残っている
- `client_context + images` をどう observation summary に圧縮するかが、現状の最大の固定ロジック領域である
- capture protocol や scheduler を変えずに、判断品質だけを次段で引き上げられる
