...
現状では、Phase 1 から Phase 4 までを完了した。
`desktop_watch` の観測意味理解は別件として保留し、この移行計画は Phase 4 でクローズする。
...
- capture protocol、client 接続、timeout は引き続きロジックに残す
- `client_context` と image をどう観測意味へ変換するかは、今回の計画外として保留する
- 必要になった時点で別計画と専用設計書へ切り出す
...
`desktop_watch` の観測意味理解は image を含み、role / payload / client 契約の設計負荷が高いため、
今回の移行計画からは外して別件保留とする。
...
- [09_RecallPackのLLM選別.md](../design/memory/09_RecallPackのLLM選別.md)

やること:

- retrieval で集めた候補群を LLM に渡し、section 優先度と採用候補を返させる
- source pack / structured output / repair 1 回の契約を追加する
- `conflicts.summary_text` を LLM 生成にする
- global limit、section limit、dedupe はコード側で最後に強制する
- failure は `retrieval_runs`、`recall_trace`、audit event へ露出する
...
### Phase 4: 自発判断の pending-intent 候補選択を LLM 化する

現状: 完了

詳細設計:

- [15_保留意図候補のLLM選別.md](../design/15_保留意図候補のLLM選別.md)
...
- `selected_candidate_ref | none` を structured output で返させる
- oldest-first の固定選択をやめる
- `wake` と `desktop_watch` の trigger 差分に応じて選択結果を適用する
...
### 保留事項: `desktop_watch` の観測意味理解

この項目は今回の LLM寄せ移行計画には含めない。

- 現状どおり `desktop_watch` は `client_context` 主体の観測文を使う
- capture image を含む観測意味理解は、必要になった時点で別計画として扱う
- その際に専用 role、source pack、inspection 契約を改めて設計する
...
- Phase 3: [09_RecallPackのLLM選別.md](../design/memory/09_RecallPackのLLM選別.md)
- Phase 4: [15_保留意図候補のLLM選別.md](../design/15_保留意図候補のLLM選別.md)

今回の計画では、これ以上 phase を追加しない。
`desktop_watch` 観測意味理解は別計画へ切り出す。
...
この移行計画は Phase 4 完了でクローズする。

理由は次のとおりである。

- 現行計画で対象にした文面生成、想起選別、自発再介入の候補選択までが完了している
- `desktop_watch` の観測意味理解は image を含む別件であり、同じ移行計画に無理に含めないほうが境界が明快である
- deterministic な runtime 境界を保ったまま、今回の LLM 寄せの目的は達成できている
