...
次にやるべき実装は Phase 4 である。

理由は次のとおりである。

- Phase 1 から Phase 3 までの recall 系移行が揃い、次は自発再介入の選択品質を上げる段階に入れる
- `desktop_watch` の画像意味理解より境界が軽く、wake の deterministic 制約を残したまま進めやすい
- 保留候補の oldest-first は現在も固定ロジックで、意味判断の LLM 化対象として次に素直である
