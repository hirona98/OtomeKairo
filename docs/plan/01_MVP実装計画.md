...
要するに、今は「会話MVP + 記憶基盤 + 自発判断 + 記憶後段非同期化の第一段」は通っており、「運用硬化・精度向上」が残っている段階である。
...
- long smoke は第五段として `--profile smoke / soak`、restart 境界、capture timeout recovery、`capture_client_id_mismatch`、`invalid_images`、`invalid_capture_error`、unknown request の無視、`desktop_watch` の reply/no-reply event 境界、複数 `vision.desktop` client 境界、seed/current editor-state 経路まで入ったが、実時間が長い soak と failure case 拡張は未完
- 仕様が固まった後の回帰テスト整備
...
1. `--profile soak` を土台に、実時間が長い soak 実行と追加 failure case へさらに広げる
2. `relationship / self` の要約精度向上と精密根拠確認を進める
3. `desktop_watch` の画像意味理解を判断へ段階的に入れる
4. 仕様が固まった範囲から回帰テストを追加する

この順にする理由は、非同期化の次は長時間運用で壊れないことを先に確認し、その上で記憶品質と観測品質を詰めるほうが安全だからである。

## 次のマイルストーン完了条件

次のマイルストーンは、運用硬化と精度向上の第一段である。
ここで完了とみなす条件は次とする。

- wake / `desktop_watch` / memory postprocess worker の長時間運用で state 境界が壊れない
- `relationship / self` の第一段要約が運用を塞がない形で安定して使える
- `desktop_watch` の観測品質改善が次段へ進める粒度で整理されている
