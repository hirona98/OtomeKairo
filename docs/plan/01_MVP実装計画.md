...
- `mentioned_entities` を query に強く効かせる
- `mentioned_topics` を query に強く効かせる
- `primary_intent` ごとの query 重みを少し明確にする

ここは `sqlite-vec` 本体の導入ではなく、既存の連想レーンの精度改善である。
...
1. `mentioned_entities / mentioned_topics` を使う連想レーン query 強化
2. `future_act` を内部結果として導入
3. runtime-only の `future_act` 候補キューを導入
4. `wake_policy` と起床判断ループを通す
5. `desktop_watch` を観測源として繋ぐ
6. 非同期ジョブ化と運用硬化へ進む
