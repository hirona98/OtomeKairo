...
| 状態層 | `world_state`、`runtime_state`、`capability_state`、`drive_state`、`ongoing_action`、設定、保留意図を判断と更新に使う | `drive_state` の第一段更新と判断文脈投入、`ongoing_action` の保存 / status / inspection 参照はあるが、能力実行の状態遷移と他状態層の広がりは未完 | 一部実装 |
...
| `ongoing_action` 参照基盤 | 継続中能力実行の有無と要約を status / inspection / 判断文脈へ入れる | 保存層、`runtime_summary.ongoing_action_exists`、inspection 参照まで入っている。状態遷移は未実装 | 一部実装 |
...
- `ongoing_action` の作成 / 継続 / 完了 / 中断の状態遷移
