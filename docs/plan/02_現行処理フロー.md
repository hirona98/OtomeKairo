...
- `wake`
  ...
  - `real-llm-smoke` は probe seed を隔離データへ入れ、`thin-drive-vision-probe / schedule-grounded-reply / social-grounded-reply / background-weak-noop / ongoing-waiting-noop` の 5 ケースで `initiative_context` と最終 decision を確認し、compact digest を `summary.json` に残す
