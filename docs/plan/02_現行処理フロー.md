...
- capability result
  ...
  - 現行の concrete result endpoint は `vision.capture / external.status / schedule.status / device.status / body.status / environment.status / location.status` があり、accepted 後の follow-up 処理は capability_id ベースで共通化している
...
`schedule.status` result の `schedule_summary / schedule_slots` は `schedule_context` へ、`device.status` result の `device_state_summary` は `device_context` へ、`body.status` result の `body_state_summary` は `body_context` へ、`environment.status` result の `environment_summary` は `environment_context` へ、`location.status` result の `location_summary` は `location_context` へ投影してから `world_state` 更新へ渡す。
capability result の `client_context.body_state_summary / device_state_summary / schedule_summary / environment_summary / location_summary` は、それぞれ `body_context / device_context / schedule_context / environment_context / location_context` の state-type 別 field として投影してから `world_state` 更新へ渡す。
