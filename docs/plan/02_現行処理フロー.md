...
| capability | `CapabilityManifest / CapabilityBinding / CapabilityState` を分離し、`hello.caps` は `{id, version}` の binding 候補とする | stream handshake、実行先選択、権限、`capability_state` を同じ契約へ揃える |
