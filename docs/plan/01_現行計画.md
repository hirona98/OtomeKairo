...
この文書は、OtomeKairo の現在実装について「今どこまで通っていて、どの段階まで完了しているか」を管理するための計画書である。
...
この前提では、会話基盤、記憶基盤、自発判断、記憶後段非同期化、`desktop_watch` 第一段、長時間 smoke までは完了している。

- `desktop_watch` は現行段階では `client_context` 主体とし、capture image の意味理解は後続拡張として扱う
- 長時間 smoke は完了済みとし、追加の長時間 soak 拡張は後続拡張として扱う
- 回帰テスト整備は現行計画の完了条件に含めない

要するに、今は主要な会話系基盤と観測系基盤は通っており、残りは完成形との差分を埋める拡張段階である。
...
## 保留中の拡張: `desktop_watch` の高度化

`desktop_watch` は第一段まで完了しており、以降は拡張テーマとして扱う。
...
個別拡張はいったん保留し、意味判断をできる限り LLM に寄せる移行を優先した。
...
今回の前提では、直近の大きなマイルストーンは次を満たした時点で完了とみなす。
...
上の条件はすでに満たしているため、以降は完成形との差分を埋める拡張フェーズとして扱う。
