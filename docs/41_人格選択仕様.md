...
- 各短周期では、`self_state`、`current_emotion`、`relationship_overview`、`drive_state` から `selection_profile` を組み立て、stable な好悪は別断面の `stable_preferences` を使う
...
- stable な好悪は、`stable_preference_projection` の confirmed / revoked 項目から再構成した `stable_preferences.likes / dislikes / revoked` を正本にする
- 初期実装で `attention_set`、`self_initiated`、`validate_action`、planner、renderer が直接使うのは、`stable_preferences` のうち `domain=action_type` と `domain=observation_kind` の項目である
- current 実装では、prompt-facing だけでなく内部選択ロジックも `stable_preferences` を正本にし、旧 `selection_profile.learned_preferences` 系は持たない
...
- `人格整合性` は、少なくとも `trait_alignment`、`style_alignment`、`relationship_alignment`、`preference_alignment`、`aversion_penalty`、`emotion_alignment`、`long_mood_alignment`、`drive_alignment` の 8 軸で評価する
...
- 初期実装の `aversion_penalty` は、`stable_preferences.dislikes` の `action_type` / `observation_kind` 一致、`stable_preferences.revoked` の弱い再抑制、`habit_biases.avoided_action_styles` の一致を合成してよい
...
- `long_mood_alignment` は、`action_selection_context.long_mood_state` の `primary_label`、`baseline_label`、`shock_label`、`source_affect_labels`、`stability` が候補の行動方向と噛み合うかである
- current 実装では、`curious` は `browse` / `look` を、`warm` と `calm` は `speak` / `notify` を、`guarded` と `tense` は `wait` を上げる
...
- 「強い回避傾向として固定された項目」は、`stable_preferences.dislikes[].weight >= 0.80` かつ `evidence_count >= 4` の項目だけを指す
...
- `experience_bias` は、`stable_preferences.likes`、`stable_preferences.dislikes`、`habit_biases` を使って補正する
...
- current 実装では、`selection_profile` から compact な人格要約を作り、`trait_values`、`interaction_style`、`habit_biases`、`emotion_bias`、`drive_bias` を毎周期の prompt self layer に明示展開してよい
- current 実装では、好みと回避傾向の prompt 注入は `stable_preferences` を使い、旧 `selection_profile.learned_preferences` 系を二重注入しない
...
- current 実装では、`selected_memory_pack.preference` に出た撤回済み嗜好も planner へ明示し、後段 validator だけに抑制を押し込まない
...
- `notify` は、`warmth`、`assertiveness`、`relationship_priorities` に従って候補化する
...
- `experience_fit 0.16`
- `drive_relief 0.10`
- current 実装では、`relationship_texts` と `preference_texts` を別信号として扱い、対人関係の厚みと嗜好想起を混同しない
- `expected_stability 0.08`
- `personality_fit` は、`trait_alignment`、`style_alignment`、`long_mood_alignment` の合成値として扱う
- `experience_fit` は、`preference_alignment` と `aversion_penalty` に加えて、`memory_bundle.relationship_items`、`memory_bundle.preference_items`、`memory_bundle.affective_items`、`memory_bundle.reflection_items` を反映してよい
...
- current 実装の `personality_fit` は、`trait_alignment 0.40`、`style_alignment 0.40`、`long_mood_alignment 0.20` で合成してよい
