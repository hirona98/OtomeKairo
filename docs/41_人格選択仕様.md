...
- `learned_preferences` と `learned_aversions` は、人格全体へ昇格済みの好悪を使う
- 初期実装で `validate_action` が直接使うのは、`learned_preferences` / `learned_aversions` のうち `domain=action_type` と `domain=observation_kind` の項目である
...
- 初期実装の `aversion_penalty` は、`learned_aversions` の `action_type` / `observation_kind` 一致に加え、`habit_biases.avoided_action_styles` の一致でも加点してよい
...
- `experience_bias` は、`learned_preferences`、`learned_aversions`、`habit_biases` を使って補正する
...
- 初期実装では、`attention manager` が独立していない間、現在の `attention_snapshot.primary_focus` と `current_observation` に対する人格寄りの比較値 1 件を `attention_score_breakdown` として作ってよい
...
- 初期実装では、4 種の自発候補のうち、その時点で最も強い 1 件の比較値だけを `self_initiated_score_breakdown` として持ってよい
...
- `skill_candidates` には、人格傾向と経験由来の bias に合う候補だけを含め、避ける行動様式に触れる候補は入れない
...
- 初期実装の `personality_fit` は、`trait_alignment * 0.50 + style_alignment * 0.50` として扱う
- `experience_fit` は、`preference_alignment` と `aversion_penalty` を反映する
...
- 初期実装では、`proposal.priority >= 0.85` の候補だけを緊急候補として扱ってよい
- 初期実装では、緊急でない候補のうち `aversion_penalty >= 0.85` かつ `relationship_fit <= 0.35` の候補は `reject` に回してよい
- 初期実装では、緊急でない候補のうち `aversion_penalty >= 0.70` かつ `relationship_fit <= 0.40` の候補は `hold` に回してよい
