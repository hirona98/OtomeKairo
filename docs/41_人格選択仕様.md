...
- 各短周期では、`self_state`、`current_emotion`、`relationship_overview`、`stable_preference_projection`、`drive_state` から `selection_profile` を組み立てる
...
- `learned_preferences` と `learned_aversions` は、`stable_preference_projection` の confirmed 項目から再構成した短周期の選択断面を使う
- `revoked_preferences` は、`stable_preference_projection.status = "revoked"` の項目から再構成した軽い抑制断面である
- 初期実装で `validate_action` が直接使うのは、`action_selection_context.stable_preferences` に再投影した `likes / dislikes / revoked` のうち `domain=action_type` と `domain=observation_kind` の項目である
