...
- ランタイムとの受け渡し契約は `docs/31_ランタイム処理契約.md` を見る
- SQLite の保存先は `docs/34_SQLite論理スキーマ.md` を見る
- API の path、HTTP method、入力 JSON、出力 JSON、`SSE` イベントで迷ったら、このドキュメントを正本として扱う

<!-- Block: Scope -->
## このドキュメントで固定する範囲

- 固定するのは、初期実装で提供する HTTP API と `SSE` の契約である
- 固定するのは、ブラウザ UI から使う制御面 API であり、内部 Python 関数の呼び出しではない
- 固定するのは、エンドポイントの意味、必須フィールド、主要な成功応答、主要な失敗応答である
