...
- このドキュメントは、`docs/30_システム設計.md` を、ランタイム実装直前の処理契約まで具体化した正本である
...
- 記憶の物理保存の輪郭は `docs/32_記憶設計.md` で固定し、最終的な SQL の細部は次に別ドキュメントで固定する前提とする
- `memory_jobs.payload_ref` と `job_kind` ごとの payload 詳細は `docs/33_記憶ジョブ契約.md` を正本とする
...
- `payload_ref` の解決規則と `job_kind` ごとの payload 本体は `docs/33_記憶ジョブ契約.md` に従う
...
- `ui_outbound_events` は append-only とし、ランタイムだけが追記し、Web サーバは更新しない
...
- `docs/30_システム設計.md` の各責務は、ここで定義した入出力契約で実装する
