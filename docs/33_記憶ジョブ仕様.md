...
- `embedding_sync` は、`write_memory` または `quarantine_memory` が検索対象の更新を確定したときだけ followup enqueue する
...
- `refresh_preview`、`quarantine_memory`、`embedding_sync` は、原則として `write_memory` の followup job として投入する
