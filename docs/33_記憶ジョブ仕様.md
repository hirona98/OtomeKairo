...
- `embedding_sync` は、`refresh_preview` または `quarantine_memory` が検索対象の更新を確定したときだけ followup enqueue する
...
- `embedding_sync` は、原則として `refresh_preview` または `quarantine_memory` の followup job として投入する
