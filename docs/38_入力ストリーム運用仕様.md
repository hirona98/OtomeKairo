...
- 固定するのは、`client_message_id` の重複受付規則、`cancel` の解決規則、会話履歴の復元元、`ui_outbound_events` の保持と削除、`SSE` 再接続時の補助挙動である
...
- HTTP path、method、各 API の成功応答は `docs/35_WebAPI仕様.md` を正本とする
- JSON のキー、型、必須項目は `docs/36_JSONデータ仕様.md` を正本とする
- ランタイム内部の処理順、claim / finalize の責務は `docs/31_ランタイム処理仕様.md` を正本とする
- テーブルやカラムの物理定義は `docs/34_SQLite論理スキーマ.md` を正本とする

<!-- Block: Current Input Kinds -->
## current の入力種別

- current の `pending_inputs` でこの運用に乗るのは `chat_message`、`microphone_message`、`camera_observation`、`network_result`、`idle_tick`、`cancel` である
- `client_message_id` の重複規則が直接かかるのは `chat_message` だけである
- `microphone_message`、`camera_observation`、`network_result`、`idle_tick`、`cancel` は current 実装では `client_message_id` を持たない
...
<!-- Block: Runtime Input Group -->
## runtime 生成入力の運用

<!-- Block: Idle Tick Rules -->
### `idle_tick`

- `idle_tick` は公開 API ではなく、ランタイムが通常 work なしで `runtime.idle_tick_ms` 経過したときだけ内部生成してよい
- `idle_tick` は、`source=idle_tick`、`trigger_reason=idle_tick`、正の `idle_duration_ms` を持つ `pending_inputs` として積む
- `idle_tick` の優先度は `chat_message` や `network_result` より低くしてよく、current 実装では `10` に固定する
- ランタイムは `idle_tick` 待機中でも短い poll 間隔で `pending_inputs` と関連 queue を再確認し、先に外部入力が来た場合はその入力を優先して処理しなければならない
- `idle_tick` を積んだ直後に高優先度の入力が先に claim された場合、その `idle_tick` は `discarded` / `discard_reason=idle_tick_superseded` にして残さない
...
- 有効な `cancel` を受理した場合、ランタイムは対象 `message_id` に対して `finish_reason=\"cancelled\"` の `message_end` を必ず 1 回だけ出す
...
<!-- Block: Canonical History -->
### 会話履歴の正本

- 会話履歴の正本は `ui_outbound_events` ではなく、`user` 側は `pending_inputs`、`assistant` 側は `action_history` に置く
- `GET /api/chat/history` は、この正本から `message(role=user|assistant)` の会話列を復元して返す
- `ui_outbound_events` は、初期履歴の正本ではなく、`SSE` 配信と再接続カーソルのための append-only 配信窓として扱う
- current 実装では、`chat_message` と `microphone_message` の受理時に、UI 即時反映用の `message(role=user)` を `ui_outbound_events` にも追記してよい
...
- 初回接続では、直前の `GET /api/chat/history` が返した `stream_cursor` を `after_event_id` として渡してよい
