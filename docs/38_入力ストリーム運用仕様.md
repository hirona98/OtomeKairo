...
- current の `pending_inputs` でこの運用に乗るのは `chat_message`、`camera_observation`、`network_result`、`idle_tick`、`cancel` である
- `client_message_id` の重複規則が直接かかるのは `chat_message` だけである
- `camera_observation`、`network_result`、`idle_tick`、`cancel` は current 実装では `client_message_id` を持たない
