...
- 初期実装の `browser_chat` では、`chat_message -> web_input`、`camera_observation(source=camera) -> camera`、`camera_observation(source=post_action_followup) -> post_action_followup`、`network_result -> network_result` に正規化する
