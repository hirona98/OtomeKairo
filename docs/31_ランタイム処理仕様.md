...
- `cognition_input` は、`cycle_meta`、`time_context`、`self_snapshot`、`behavior_settings`、`selection_profile`、`body_snapshot`、`world_snapshot`、`drive_snapshot`、`task_snapshot`、`attention_snapshot`、`attention_score_breakdown`、`self_initiated_score_breakdown`、`memory_bundle`、`policy_snapshot`、`skill_candidates`、`current_observation`、`context_budget` を持つ
...
- 初期実装では、`attention_score_breakdown` は現在の `attention_snapshot.primary_focus` と `current_observation` に対する人格寄りの比較値を 1 件だけ持ってよい
...
- 初期実装では、`self_initiated_score_breakdown` は 4 種の自発候補のうち、その時点で最も強い 1 件の人格寄り比較値を持ってよい
- `skill_candidates` には、今回の状況に適合しうるスキルだけを含める
- 初期実装では、`skill_candidates` は `selection_profile.learned_preferences` と `habit_biases` から作る `skill_candidate_entry` の配列でよい
...
- 初期実装では、永続 `skill_registry` が無い間、`context assembler` は `selection_profile.learned_preferences` と `habit_biases` から人格に合う `skill_candidates` を組み立ててよい
