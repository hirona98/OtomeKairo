...
- current の `browser_chat` では、`action validator` は `action_selection_context` の `fact_entries` / `relationship_texts` / `preference_texts` / `affect_entries` / `reflection_entries` / `recent_dialog` と、直近の `self_snapshot.last_persona_update` を使って候補差を付けてよい
- 安全に反する候補は、その時点で棄却する
