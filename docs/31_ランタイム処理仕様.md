...
- current 実装では、`event_preview_cache.preview_text` が存在する event について、`recent_event_window` と `episodic_event` の selector 候補文面に preview を優先して使ってよい
...
- `context assembler` は、その選別結果を `retrieval_runs.selected_json` と `retrieval_context.selected` の両方へ保存できる shape にまとめる
