...
- current の `run_once()` は、`settings_change_sets`、`settings_overrides`、`pending_inputs`、`waiting_external` の `browse` task、`memory_jobs` の順で 1 件ずつ処理する
- `pending_inputs` の current 入力種別は `chat_message`、`camera_observation`、`network_result`、`cancel` に限る
- `chat_message`、`camera_observation`、`network_result` は、`input_journal` 追記、`build_cognition_input`、`LiteLLM` 認知、`validate_action`、`dispatch_action_command`、`finalize_pending_input_cycle` の順で処理する
...
- `microphone` の観測回収、`SNS` 入出力、`move`、`social`、`idle_tick` 起点の自発行動起動は current では未実装である
