...
- current の認知入力は `current_observation`、`selection_profile`、`attention_snapshot`、決定論的に組んだ `memory_bundle`、prompt 向けの `recent_dialog` と `selected_memory_pack`、人格の固定断面である `stable_self_state`、`confirmed_preferences`、`long_mood_state`、`reply_render_input`、`task_snapshot`、`camera_candidates` を含む
...
- current 実装の `context assembler` は、`selection_profile` から compact な人格要約を組み、`trait_values`、`interaction_style`、`learned_preferences`、`learned_aversions`、`revoked_preferences`、`habit_biases`、`emotion_bias`、`drive_bias` を毎周期の prompt self layer と `context_budget.self` の見積りで共有してよい
...
- `confirmed_preferences` には、`state_snapshot.stable_preference_items` から抽出した `status=confirmed` の `preference` だけを含める
...
- current 実装では、`memory_snapshot.relationship_items` に入れる `preference` は `preference_memory` の直近窓ではなく、`stable_preference_projection` を retrieval 用件数で再読した read model を使ってよい
...
- current 実装では、`recent_dialog` は `chat_message` / `microphone_message` と `external_response` の列だけを時系列に戻して持ち、`selected_memory_pack` は `recent_context`、`working_memory`、`episodic`、`facts`、`affective`、`relationship`、`reflection` に分けてよい
- current 実装では、`action_selection_context` は `recent_context_texts`、`working_memory_texts`、`episodic_texts`、`fact_entries`、`affect_entries`、`relationship_texts`、`reflection_entries`、`recent_dialog`、`stable_preferences`、`long_mood_state` を持ってよい
...
- current の `browser_chat` では、`action validator` は `action_selection_context` の `fact_entries` / `relationship_texts` / `affect_entries` / `reflection_entries` / `recent_dialog` と、直近の `self_snapshot.last_persona_update` を使って候補差を付けてよい
- 安全に反する候補は、その時点で棄却する
