...
- current 実装の `retrieval_context.plan.collector_names` には、`recent_event_window`、`associative_memory`、`episodic_memory` に加えて、`reply_chain`、`context_threads`、`state_link_expand`、必要なら `relationship_focus`、`task_focus`、`reflection_focus`、`explicit_time` を含めてよい
