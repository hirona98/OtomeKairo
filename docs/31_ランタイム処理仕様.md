...
- current の認知入力は `current_observation`、`selection_profile`、`attention_snapshot`、決定論的に組んだ `memory_bundle`、prompt 向けの `recent_dialog` と `selected_memory_pack`、人格の固定断面である `stable_self_state`、`stable_preferences`、`long_mood_state`、`reply_render_input`、`task_snapshot`、`camera_candidates` を含む
...
flowchart TD
    ...
    claim --> maintain["reflect + consolidate + sync"]
    maintain --> idle
```
...
- 1 回の長周期ループでは、`claimed_memory_jobs`、`reflection_bundle`、`memory_updates`、`embedding_updates` を作る
...
- `self_state` は、現在の性格傾向、現在感情、長期目標、関係性、人格としての不変条件を持つ
...
- `source` は、少なくとも `web_input`、`camera`、`microphone`、`network_result`、`idle_tick`、`post_action_followup` を区別する
...
- `kind` は、少なくとも `instruction`、`dialogue_turn`、`scene_change`、`audio_segment`、`search_result`、`internal_trigger` を区別する
...
- `selection_profile` は、`self_state`、`current_emotion`、`relationship_overview`、`drive_state` から再構成する
- `selection_profile` は、少なくとも `trait_values`、`interaction_style`、`relationship_priorities`、`habit_biases`、`emotion_bias`、`drive_bias` を持つ
...
- current 実装では、`action validator` は `action_selection_context.long_mood_state` を `long_mood_alignment` へ写像し、`personality_fit` の短期補正として使ってよい
...
- `cognition_input` は、`cycle_meta`、`time_context`、`self_snapshot`、`stable_self_state`、`stable_preferences`、`long_mood_state`、`behavior_settings`、`selection_profile`、`body_snapshot`、`world_snapshot`、`drive_snapshot`、`task_snapshot`、`attention_snapshot`、`memory_bundle`、`recent_dialog`、`selected_memory_pack`、`action_selection_context`、`retrieval_context`、`policy_snapshot`、`camera_candidates`、`skill_candidates`、`current_observation`、`context_budget`、`reply_render_input` を持つ
...
- `self_snapshot` には、性格傾向、現在感情、長期目標、関係性、人格としての不変条件、好みの行動様式を含める
- `behavior_settings` には、呼び方、振る舞い本文、追加指示、応答ペース、自発性、検索傾向、通知傾向、話し方、詳細さを含める
- `selection_profile` には、その短周期での人格選択に使う短期補正済みの選択断面を含める
- current 実装の `context assembler` は、`selection_profile` から compact な人格要約を組み、`trait_values`、`interaction_style`、`habit_biases`、`emotion_bias`、`drive_bias` を prompt self layer と `context_budget.self` の見積りで共有してよい
- current 実装では、好みと回避傾向の prompt 注入は `selection_profile` ではなく `stable_preferences` を正本にしてよい
...
- `stable_self_state` には、現在感情ラベル、長期目標要約、関係性要約、進行中タスク要約、待機タスク要約、不変条件の要約を含める
- `stable_preferences` には、`state_snapshot.stable_preference_items` から抽出した `likes`、`dislikes`、`revoked` を含め、planner / validator / renderer の共通嗜好断面として使う
- current 実装では、`state_snapshot.stable_preference_items` は `stable_preference_projection` から `confirmed like`、`confirmed dislike`、`revoked` を bucket ごとに上位件数だけ読んだ stable read model としてよい
- current 実装では、`memory_snapshot.preference_items` に入れる `preference` は `preference_memory` の直近窓ではなく、`stable_preference_projection` を retrieval 用件数で再読した read model を使ってよい
- `long_mood_state` には、`state_snapshot.stable_long_mood_item` から抽出した最新 `memory_kind=long_mood_state` の要約断面だけを含める
...
- `recent_dialog` は、`recent_event_window` から抽出した直近会話の生系列である
- `selected_memory_pack` は、長期記憶を prompt 向けに圧縮した会話注入断面である
- `action_selection_context` は、`action validator` 専用の派生断面であり、候補比較に必要な記憶断面だけを `selected_memory_pack` と `memory_bundle` から再投影したものである
- current 実装では、`recent_dialog` は `chat_message` / `microphone_message` と `external_response` の列だけを時系列に戻して持ち、`selected_memory_pack` は `recent_context`、`working_memory`、`episodic`、`facts`、`affective`、`relationship`、`preference`、`reflection` に分けてよい
...
- current 実装では、`action_selection_context` は `recent_context_texts`、`working_memory_texts`、`episodic_texts`、`fact_entries`、`affect_entries`、`relationship_texts`、`preference_texts`、`reflection_entries`、`recent_dialog`、`stable_preferences`、`long_mood_state` を持ってよい
- current 実装では、`action validator` は stable preference を `selection_profile` からではなく `action_selection_context.stable_preferences` から読み、派生断面だけで候補比較してよい
- `retrieval_context.plan` には、その短周期で使った `RetrievalPlan` を、`retrieval_context.selected` には `LLM` selector の選別理由、件数要約、最終使用集合の `selection_trace` を含める
- current 実装の `retrieval_context.plan.collector_names` には、`recent_event_window`、`associative_memory`、`episodic_memory` に加えて、`reply_chain`、`context_threads`、`state_link_expand`、`entity_expand`、必要なら `relationship_focus`、`task_focus`、`reflection_focus`、`explicit_time` を含めてよい
- current 実装では、`retrieval_context.plan.time_hint` に `explicit_dates` を追加し、`YYYY-MM-DD` へ正規化した具体日付を持ってよい
...
- current 実装では、`context assembler` は `event_about_time` から付けた `about_time_hint_text` を `memory_bundle` と `selected_memory_pack` に反映してよい
...
- current 実装では、`memory_bundle` のうち `recent_event_window`、`reflection_items`、`episodic_items`、`semantic_items`、`affective_items`、`preference_items`、`relationship_items`、`working_memory_items` の順で末尾から落とし、人格核に近い断面を後まで残す
- current 実装では、`memory_bundle` を削った結果に合わせて `retrieval_context.selected` も最終使用集合へ詰め直す
- `reply_render_input` は、`reply_render` 専用の派生断面であり、`observation_text`、`time_reference_text`、`attention_summary_text`、`retrieval_summary_text`、`stable_self_state`、`stable_preferences`、`long_mood_state`、`recent_dialog`、`selected_memory_pack`、`reply_style` だけを持つ
- `reply_render_plan` は、`cognition_plan` から抽出した render 専用計画断面であり、`intention_summary`、`decision_reason`、`reply_mode`、`reply_reason`、`memory_focus_kind`、`memory_focus_summary`、`action_summaries` だけを持つ
- current 実装では、`reply_render` は full `cognition_input` や full `cognition_plan` ではなく、`reply_render_input` と `reply_render_plan` だけを LLM 入力に使ってよい
- current 実装では、`context assembler` は `completion_settings` を別途組み、`build_retrieval_artifacts` の中で `RetrievalPlan -> collector 群 -> merged candidates -> LLM selector -> slot limit apply -> memory_bundle` を先に確定してから `cognition_plan` へ進む
- `context assembler` は、その選別結果を `retrieval_context.selected` として後続処理へ渡せる shape にまとめる
...
- `action_type` は、少なくとも `speak`、`look`、`browse`、`notify`、`wait` を区別する
...
- current の `browser_chat` では、`action validator` は `action_selection_context` の `fact_entries` / `relationship_texts` / `preference_texts` / `affect_entries` / `reflection_entries` / `recent_dialog` を使って候補差を付けてよい
- 安全に反する候補は、その時点で棄却する
...
- current の `browser_chat` では、`attention_state`、`body_state`、`world_state`、`drive_state`、`task_state`、`action_history`、`pending_inputs`、`settings_overrides`、`events`、`memory_jobs`、`commit_records` を短周期で更新する
...
- 1 回の短周期の保存単位は、`self_state`、必要なら `runtime_settings`、`attention_state`、`body_state`、`world_state`、`drive_state`、`task_state`、`working_memory`、`recent_event_window`、`action_history`、`pending_inputs`、`settings_overrides`、`settings_change_sets`、新規に enqueue する `memory_jobs` の処理結果である
...
- `memory_jobs` は、少なくとも `write_memory`、`embedding_sync` を区別する
...
- `job_kind="write_memory"` の選別結果は、`episodic_updates`、`semantic_updates`、`affective_updates`、`relationship_updates` に分ける
...
### `skill_candidates` と学習の仕様

- current 実装では、`skill_candidates` は自発行動候補の短周期フィルタであり、永続的なスキル台帳は持たない
- `skill_candidates` は、`self_initiated_score_breakdown` から毎周期再計算する一時オブジェクトである
