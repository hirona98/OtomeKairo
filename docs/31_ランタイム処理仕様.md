...
- current 実装では、`memory_bundle.working_memory_items` に `memory_kind=summary`、`memory_bundle.semantic_items` に `memory_kind=fact`、`memory_bundle.recent_event_window` に直近 `5` 件の `searchable` な `events` を使ってよい
