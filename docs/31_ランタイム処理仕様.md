...
- 初期実装では、ランタイムの通常遷移と失敗はログへ残し、端末には `INFO` 以上、`log/otomekairo.log` には `DEBUG` の通常テキストログで残す
...
- 初期実装では、`claim -> decide -> dispatch -> finalize` の主要遷移を `INFO` ログで残し、`memory_jobs` のような細かい遷移は `DEBUG` で残してよい
...
- 初期実装では、`idle_tick` 起点の自発行動起動自体はまだ持たず、同じ比較値を `skill_candidates` の適合フィルタに使ってよい
...
- 1 回の短周期ループでは、`observation_batch`、`input_journal_batch`、`attention_set`、`selection_profile`、必要なら `attention_score_breakdown`、`self_initiated_score_breakdown`、`action_candidate_score`、`cognition_input`、`cognition_result`、`action_command`、`commit_record` を作る
...
- 初期実装の `browse` では、`waiting_external` のタスクを後続の短周期で claim し、外部検索アダプタの結果で `completed` または `abandoned` に進めてよい
- 初期実装の `browse` 成功時は、外部検索結果を内部入力 `network_result` として `pending_inputs` に enqueue し、次の短周期で再認知してよい
- 初期実装の `browse` 成功時は、`browsing`、`browse_completed`、`processing_external_result` の UI 通知を出してよい
...
- 初期実装の `browser_chat` では、`chat_message -> web_input`、`camera_observation -> camera`、`network_result -> network_result` に正規化する
- `kind` は、少なくとも `instruction`、`dialogue_turn`、`scene_change`、`audio_segment`、`search_result`、`social_reaction`、`internal_trigger` を区別する
- 初期実装の `browser_chat` では、`chat_message -> dialogue_turn`、`camera_observation -> scene_change`、`network_result -> search_result`、`cancel -> instruction` に正規化する
...
- `cognition_input` は、`cycle_meta`、`time_context`、`self_snapshot`、`behavior_settings`、`selection_profile`、`body_snapshot`、`world_snapshot`、`drive_snapshot`、`task_snapshot`、`attention_snapshot`、`memory_bundle`、`retrieval_context`、`policy_snapshot`、`skill_candidates`、`current_observation`、`context_budget` を持つ
...
- 初期実装では、`task_snapshot.active_tasks` と `task_snapshot.waiting_external_tasks` に、それぞれ優先度上位 `3` 件までの `task_state` を入れてよい
- `attention_snapshot` には、主注意対象、抑制対象、再確認候補を含める
- 初期実装の `attention_snapshot` は、`current_observation`、優先度最高の `active_task`、最優先の `relationship_priority` を比較し、`primary_focus`、`secondary_focuses`、`revisit_queue` を再構成してよい
...
- 初期実装では、`memory_bundle.working_memory_items` に `memory_kind=summary`、`memory_bundle.semantic_items` に `memory_kind=fact`、`memory_bundle.recent_event_window` に直近 `5` 件の `searchable` な `events` を使ってよい
- 初期実装では、`memory_bundle.episodic_items` は `events` を `memory_kind=episodic_event` へ正規化して使い、`memory_bundle.affective_items` は `long_mood_state` と `event_affect`、`memory_bundle.relationship_items` は `relation` と `preference_memory`、`memory_bundle.reflection_items` は `reflection_note` を使ってよい
- 初期実装では、`current_observation.observation_text` と、必要なら `query` / `source_task_id` を使って `sqlite-vec` の類似候補を先に補強し、そのうえで決定論的な一致判定で `memory_bundle` に入れる `summary`、`fact`、`recent_event_window` を絞ってよい
...
- 初期実装の `skill_candidates` は、`self_initiated_score_breakdown` の上位候補から `fit_score >= 0.30` の項目だけを採り、`suggested_action_types` を添えてよい
- `current_observation` には、その短周期を起動した観測本文と付随情報を含める
- `context_budget` には、今回の `LLM` 呼び出しで使える全体量上限と、`self`、`behavior`、`situation`、`memory`、`output contract` への割当上限を含める
...
- `context assembler` は、`RetrievalPlan -> 候補収集 -> 決定論的選別 -> memory_bundle` の結果を、そのまま `retrieval_runs` へ保存できる shape にまとめる
...
- 初期実装を含めて、`cognition_result` は認知層の正本出力として 1 回で構造化して返す
...
- 初期実装では、`LiteLLM` 呼び出しに `response_format={"type":"json_schema", ...}` を付け、`cognition_result` 全体を strict な JSON Schema で拘束する
- 初期実装では、`priority` の `0.0..1.0`、`action_type` の許可集合、`browse.query`、`speak/notify.target_channel`、`look` の必須パラメータを schema 側でも検証する
...
- 初期実装では、`speech_draft` は常に持ち、`speak`、`notify`、`look` で使うユーザー向け文言の原案を表す
...
- 初期実装の `browser_chat` では、`action_proposal` の受理対象を `speak`、`browse`、`notify`、`look`、`wait` に絞ってよい
...
- 初期実装の `browser_chat` では、`command_type` を `speak_ui_message`、`dispatch_notice`、`enqueue_browse_task`、`control_camera_look` に絞ってよい
...
- 初期実装の `browser_chat` では、`notify` の最上位候補を選んだ場合、ユーザー通知 (`notice` イベント) として実行してよい
- 初期実装の `browser_chat` では、`browse` の最上位候補を選んだ場合、`task_state(waiting_external)` へ検索タスクを enqueue してよい
- 初期実装の `browser_chat` では、`look` の最上位候補を選んだ場合、ONVIF 経由のカメラ視点操作として実行してよい
- 初期実装の `browser_chat` では、`task_state(waiting_external, task_kind=browse)` を後続の短周期で外部検索アダプタへ渡し、成功時は `network_result` を内部入力として enqueue し、失敗時だけ `error` を出してよい
- 初期実装の `browser_chat` では、`wait` の最上位候補だけを `hold` として確定してよい
- 初期実装の `browser_chat` では、`action validator` は `task_snapshot.waiting_external_tasks` に同じ `query` の `browse` がある候補を hard gate で棄却してよい
- 初期実装の `browser_chat` では、`action validator` は `memory_bundle` の `summary` / `fact` / `relation` / `preference` / `event_affect` / `reflection_note` と、直近の `self_snapshot.last_persona_update` を使って候補差を付けてよい
- 安全に反する候補は、その時点で棄却する
...
- 初期実装の `browser_chat` では、`attention_state`、`task_state`、`action_history`、`pending_inputs`、`settings_overrides`、`events`、`memory_jobs`、`retrieval_runs`、`commit_records` を短周期で更新し、`body_state`、`world_state`、`drive_state` は seed 済み singleton をそのまま参照してよい
...
- 初期実装の `browser_chat` では、この段階で少なくとも `attention_state` と `task_state` を確定し、他の live state は seed 済み singleton を維持してよい
...
- 初期実装では、`skill_registry` への昇格処理はまだ持たず、`skill promoter` は `cognition_input.skill_candidates` を作る人格適合フィルタとして実装してよい
...
- `camera api` の `observe` は、撮影した静止画を `self_initiated` の `camera_observation` として `pending_inputs` に書き込む
...
- `camera_observation` は、`camera_still_image` 添付を 1 件以上必須とし、`text` を持たない自発観測入力として扱う
- `ui_outbound_events.event_type` は、少なくとも `token`、`message`、`status`、`notice`、`error` を区別する
...
- 認知層は `cognition_result` を返すが、`action_command` は返さない
