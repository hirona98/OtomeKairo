...
- `pending_inputs` の current 入力種別は `chat_message`、`camera_observation`、`network_result`、`idle_tick`、`cancel` に限る
- `chat_message`、`camera_observation`、`network_result`、`idle_tick` は、`input_journal` 追記、`build_cognition_input`、`LiteLLM` 認知、`validate_action`、`dispatch_action_command`、`finalize_pending_input_cycle` の順で処理する
- `cancel` は、現在進行中の `browser_chat` 応答だけを対象に解決し、対象がなければ `discarded` に確定する
- current の認知入力は `current_observation`、`selection_profile`、`attention_snapshot`、決定論的に組んだ `memory_bundle`、`task_snapshot` を含む
...
- current の `ui_outbound_events` は `status`、`token`、`message`、`notice`、`error` を使い、`status api` とは別の制御面ストリームとして扱う
- `microphone` の観測回収、`SNS` 入出力、`move`、`social` は current では未実装である
...
- 初期実装の `browser_chat` では、`chat_message -> web_input`、`camera_observation(source=camera) -> camera`、`camera_observation(source=post_action_followup) -> post_action_followup`、`network_result -> network_result`、`idle_tick -> idle_tick` に正規化する
- `kind` は、少なくとも `instruction`、`dialogue_turn`、`scene_change`、`audio_segment`、`search_result`、`social_reaction`、`internal_trigger` を区別する
- 初期実装の `browser_chat` では、`chat_message -> dialogue_turn`、`camera_observation -> scene_change`、`network_result -> search_result`、`idle_tick -> internal_trigger`、`cancel -> instruction` に正規化する
...
- current 実装では、`POST /api/camera/observe` は `source=camera` / `trigger_reason=self_initiated`、`look` 後の追跡観測は `source=post_action_followup` / `trigger_reason=post_action_followup`、`idle_tick` は `source=idle_tick` / `trigger_reason=idle_tick` を使う
...
- current 実装では、`priority` の `0.0..1.0`、`action_type` の許可集合、`browse.query`、`speak/notify.target_channel`、`look` の必須パラメータを schema 側でも検証する
...
- `camera_observation` は、`camera_still_image` 添付を 1 件以上必須とし、`text` を持たない観測入力として扱う
