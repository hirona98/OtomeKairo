...
- `chat_message`、`microphone_message`、`camera_observation`、`network_result`、`idle_tick` は、`input_journal` 追記、`build_cognition_input`、`cognition_plan` 生成、`reply_render`、`validate_action`、`dispatch_action_command`、`finalize_pending_input_cycle` の順で処理する
...
- 1 回の短周期ループでは、`observation_batch`、`input_journal_batch`、`attention_set`、`selection_profile`、必要なら `attention_score_breakdown`、`self_initiated_score_breakdown`、`action_candidate_score`、`cognition_input`、`cognition_plan`、`speech_draft`、`cognition_result`、`action_command`、`commit_record` を作る
...
- current 実装では、`cognition_plan` は `intention_summary`、`decision_reason`、`action_proposals`、`step_hints`、`memory_focus`、`reflection_seed` を持つ
