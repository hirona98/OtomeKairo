...
- `confirmed_preferences` には、`memory_snapshot.relationship_items` から抽出した `status=confirmed` の `preference` だけを含める
- `long_mood_state` には、`memory_snapshot.affective_items` から抽出した最新 `memory_kind=long_mood_state` の要約断面だけを含める
