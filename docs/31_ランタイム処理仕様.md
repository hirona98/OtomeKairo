...
- current の認知入力は `current_observation`、`selection_profile`、`attention_snapshot`、決定論的に組んだ `memory_bundle`、prompt 向けに圧縮した `conversation_context`、`task_snapshot`、`camera_candidates` を含む
...
- `conversation_context` には、`recent_event_window` から抽出した `recent_dialog` と、長期記憶を prompt 向けに圧縮した `selected_memory_pack` を含める
...
- current 実装では、`context assembler` は `event_about_time` から付けた `about_time_hint_text` を `memory_bundle` と `conversation_context.selected_memory_pack` に反映してよい
