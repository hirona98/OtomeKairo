...
- `reply_render_input` は、`reply_render` 専用の派生断面であり、`current_observation`、`time_context`、`attention_snapshot`、`retrieval_context`、`stable_self_state`、`confirmed_preferences`、`long_mood_state`、`recent_dialog`、`selected_memory_pack`、`reply_style` だけを持つ
- current 実装では、`reply_render` は full `cognition_input` ではなく `reply_render_input` だけを LLM 入力に使ってよい
