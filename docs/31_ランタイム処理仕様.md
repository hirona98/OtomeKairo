...
- `cognition_input` は、`cycle_meta`、`time_context`、`self_snapshot`、`behavior_settings`、`selection_profile`、`body_snapshot`、`world_snapshot`、`drive_snapshot`、`task_snapshot`、`attention_snapshot`、`memory_bundle`、`policy_snapshot`、`skill_candidates`、`current_observation`、`context_budget` を持つ
...
- 初期実装の `browser_chat` では、`action_proposal` の受理対象を `speak`、`browse`、`notify`、`wait` に絞ってよい
...
- 初期実装の `browser_chat` では、`command_type` を `speak_ui_message`、`dispatch_notice`、`enqueue_browse_task` に絞ってよい
...
- 初期実装の `browser_chat` では、`action validator` は `memory_bundle` の `summary` / `fact` を使って、既知の検索対象への `browse` を下げ、`network_result` 後の `speak` / `notify` を上げてよい
- 安全に反する候補は、その時点で棄却する
...
- 1 回の短周期の保存単位は、`self_state`、必要なら `runtime_settings`、`attention_state`、`body_state`、`world_state`、`drive_state`、`task_state`、`working_memory`、`recent_event_window`、`action_history`、`pending_inputs`、`settings_overrides`、新規に enqueue する `memory_jobs`、必要なら `retrieval_runs` の処理結果である
...
- 次に、入力の取り込み結果として `pending_inputs` と `settings_overrides` の状態を更新し、`runtime` scope の設定反映がある場合は `runtime_settings` も更新する
...
- `settings api` と `chat input api` は、人格状態を直接変更せず、`pending_inputs` と `settings_overrides` に要求を書き込む
...
- 例外として、古い `ui_outbound_events` の保持期限切れ削除は、`docs/38_入力ストリーム運用仕様.md` に従って Web サーバ側の stream janitor だけが行ってよい
...
- ランタイムは、`apply_scope="runtime"` の `applied` を同じ短周期で `runtime_settings` に反映する
- ランタイムは、`apply_scope="next_boot"` の `applied` を同じ短周期では `runtime_settings` に反映せず、次回起動時に materialize する
