...
- 初期実装では、ランタイムの通常遷移と失敗はログへ残し、端末 / ファイル / library logger の level は `config/developer.toml` で固定する
...
- `chat_message`、`microphone_message`、`camera_observation`、`network_result`、`idle_tick` は、`input_journal` 追記、`build_cognition_input`、`LiteLLM` 認知、`validate_action`、`dispatch_action_command`、`finalize_pending_input_cycle` の順で処理する
...
- current の `camera_candidates` には、enabled なカメラ一覧に加えて、camera adapter が live に解決した `presets[]` を含めてよい
...
- `browse` を確定した場合は、まず `waiting_external` task と `browse_queued` notice を作り、後続ループで task を claim して外部検索し、`browse_completed` notice と `network_result` を返す
...
- current の scheduler は、短周期の直後に `memory_jobs` を 1 件優先するため、`post_action_followup` の `camera_observation` は通常「次の iteration の次」で消費される
...
- current 実装の `browse` 成功時は、`browsing`、`browse_completed`、`processing_external_result` の UI 通知を出してよい
...
- current 実装では、`POST /api/microphone/input` は raw audio body を同期 `STT` して `source=microphone` / `trigger_reason=external_input` の `microphone_message` を enqueue する
- current 実装では、`POST /api/camera/observe` は `camera_connection_id` を明示指定した enabled camera connection を `source=camera` / `trigger_reason=self_initiated` で流し、`look` 後の追跡観測は `source=post_action_followup` / `trigger_reason=post_action_followup`、`idle_tick` は `source=idle_tick` / `trigger_reason=idle_tick` を使う
...
- current 実装では、`memory_bundle.working_memory_items` に `memory_kind=summary`、`memory_bundle.semantic_items` に `memory_kind=fact`、`memory_bundle.recent_event_window` に active memory preset の `retrieval_profile.recent_window_limit` 件までの `searchable` な `events` を使ってよい
- current 実装では、`memory_bundle.episodic_items` は `events` を `memory_kind=episodic_event` へ正規化して使い、`memory_bundle.affective_items` は `long_mood_state` と `event_affect`、`memory_bundle.relationship_items` は `relation` と `preference_memory`、`memory_bundle.reflection_items` は `reflection_note` を使ってよい
- current 実装では、`current_observation.observation_text` と、必要なら `query` / `source_task_id` を使って `sqlite-vec` の類似候補を先に補強し、そのうえで決定論的な一致判定で `memory_bundle` に入れる `summary`、`fact`、`recent_event_window` を絞ってよい
...
- current 実装では、`LiteLLM` 呼び出しに `response_format={"type":"json_schema", ...}` を付け、`cognition_result` 全体を strict な JSON Schema で拘束する
- current 実装では、`LiteLLM` 呼び出しに `base_model` へ設定上の `llm.model` と同じ値を渡し、OpenRouter が返す日付付き実モデル名ではなく設定モデル名で capability / cost の参照を安定化する
- current 実装では、`priority` の `0.0..1.0`、`action_type` の許可集合、`browse.query`、`speak/notify.target_channel`、`look.camera_connection_id` と `look` の視点操作パラメータを schema 側でも検証する
...
- current 実装では、`speech_draft` は常に持ち、`speak`、`notify`、`look` で使うユーザー向け文言の原案を表す
- current 実装では、`look` を提案する場合の `speech_draft` は視点変更中の案内文として使ってよく、同じ内容の `speak` 候補は重複候補として返さないことを prompt で求める
...
- current の `browser_chat` では、`browse` の最上位候補を選んだ場合、`task_state(waiting_external)` へ検索タスクを enqueue してよい
- current の `browser_chat` では、`look` の最上位候補を選んだ場合、ONVIF 経由のカメラ視点操作として実行してよい
- current の `browser_chat` では、`camera_candidates` は `camera_connections[].is_enabled=true` の一覧を camera adapter が解決した live 候補とし、各候補に `presets[]` を含めてよい
...
- current 実装では、実行可能な `look` 候補がある短周期では、`control_camera_look` 自身が `speech_draft` を `message` として出せるため、同周期の `speak` 候補を validator で重複排除する
- current 実装では、`camera_candidates[].presets[]` がある場合、広い視点変更や別方向確認は `preset_name` を優先し、`direction` はプリセットがない場合か微調整に使ってよい
- current 実装では、`direction` の fallback move は ONVIF `ContinuousMove` を固定速度で一定時間だけ流す単発ステップとし、広すぎない範囲で従来より大きめの 1 手を使ってよい
- current の `browser_chat` では、`task_state(waiting_external, task_kind=browse)` を後続の短周期で外部検索アダプタへ渡し、成功時は `network_result` を内部入力として enqueue し、失敗時だけ `error` を出してよい
- current の `browser_chat` では、`wait` の最上位候補だけを `hold` として確定してよい
...
- current 実装では、`camera_observation` の `source` は `camera` または `post_action_followup` に固定し、起動理由は `trigger_reason` で区別する
