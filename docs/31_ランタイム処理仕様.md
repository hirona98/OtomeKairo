...
- current 実装では、`action_selection_context` は `recent_context_texts`、`working_memory_texts`、`episodic_texts`、`fact_entries`、`affect_entries`、`relationship_texts`、`reflection_entries`、`recent_dialog`、`confirmed_preferences`、`long_mood_state` を持ってよい
...
- `reply_render_input` は、`reply_render` 専用の派生断面であり、`observation_text`、`time_reference_text`、`attention_summary_text`、`retrieval_summary_text`、`stable_self_state`、`confirmed_preferences`、`revoked_preferences`、`long_mood_state`、`recent_dialog`、`selected_memory_pack`、`reply_style` だけを持つ
