...
- まず `memory_write_e2e` の 6 cycle scripted conversation を実行する
...
- `action_transparency_visible`: `response_action_transparency_cycle_count >= 3`
- `failure_explanation_visible`: `response_failure_explanation_cycle_count >= 1`
- `preference_reference_visible`: `response_preference_reference_cycle_count >= 2`
- `controlled_preference_violation`: `response_preference_violation_cycle_count <= 1`
- `mood_tone_hint_visible`: `response_mood_tone_hint_cycle_count >= 3`

<!-- Block: Reading -->
## 読み方

- ここで落ちるなら、`write_memory`、`selection_profile`、`reply render`、`chat_replay_eval` のどこかが会話品質を壊している
- `controlled_preference_violation` を 1 件まで許しているのは、現在の scripted scenario に restore 用の `browse` cycle が含まれているためである
