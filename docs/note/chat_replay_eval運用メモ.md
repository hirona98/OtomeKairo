...
- 直近の `chat_message` cycle を時系列で replay し、`dialogue thread`、`preference carryover`、`long_mood carryover` の効き方を確認する
...
- `action_history` から、その cycle の行動種別を取り出して prior preference と照合する
