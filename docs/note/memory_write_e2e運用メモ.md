...
- 4 cycle の scripted conversation を使う
...
- `write_memory` 4 件、`refresh_preview` は `events` 件数と一致、`embedding_sync` は `write_memory + refresh_preview` 件数と一致
- `memory_states` に `summary`、`fact`、`reflection_note`、`long_mood_state` が入る
- `preference_memory` は最終的に `confirmed=2`、`revoked=2` になる
