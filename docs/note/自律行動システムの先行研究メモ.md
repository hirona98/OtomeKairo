...
- `Voyager`
  ...
  - 設計示唆:
    - 成功した行動列の再利用根拠は `task_state` と別で持つ
...
- `人間に近い存在` を作るには、`記憶` だけでなく `注意`, `行動選択`, `再利用可能な行動パターン`, `身体への接地`, `反省`, `安全制御` が必要である
...
- 次に詳細設計へ反映すべきなのは、`attention`, `reflection`, `affordance-aware world_state`, `instruction priority` の 4 点である
