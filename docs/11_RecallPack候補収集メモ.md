...
- 構造レーンを主にしつつ、連想補助として小さいベクトルレーンを持つ
- 多段展開や無制限探索は、同期想起の標準経路に入れない

<!-- Block: TwoLanes -->
## 同期想起は 2 レーンで行う

OtomeKairo では、同期想起を次の 2 レーンに分けるのがよい。

- 構造レーン:
  - `scope`、`memory_type`、`status` に基づく厳密な収集
  - `active_commitments`、`relationship_model`、明示的な `user` / `self` 理解を主に担う
- 連想レーン:
  - 現在入力と直近会話から作った問い合わせで、`memory_units` と `episode_digests` をベクトル検索する
  - 曖昧な連想、似た空気感、前にもあった話題の再浮上を補助する

重要なのは、ベクトルレーンを主役にしないことである。
人格の連続性に直結する `commitment` や `relationship` は、構造レーンを主に使い、ベクトルレーンは補助候補として扱う。
...
2. 構造レーンで hard requirement 候補を集める
3. 連想レーンで補助候補を集める
4. `RecallPack` の各 section へ専用ルールで再配置する
5. section ごとの上限と全体上限をかける
6. 最後に `conflicts` を付与する
...
この `RecallHint` は、現在のユーザー入力と直近会話ウィンドウを軽量高速 LLM へ渡して作る。
長期記憶そのものは、ここにはまだ混ぜない。
詳細な生成規則は `12_RecallHint生成メモ.md` で別途整理する。
...
連想レーンは、曖昧に自己傾向を思い出す補助としては使ってよい。
ただし、ベクトル hit だけで強い `self_model` を作らないほうがよい。
...
ここでは、連想レーンを補助的に使ってよい。
たとえば、明示話題に近いが完全一致しない `memory_units` を、ベクトル hit から追加候補として拾うのは自然である。
ただし、vector だけで拾った候補は、構造条件に合う候補より弱く扱う。
...
連想レーンは、曖昧な気まずさや似た相談文脈を補助的に拾うためには使ってよい。
ただし、ベクトル hit だけで強い関係変化を断定しない。
...
`active_topics` は、連想レーンの恩恵を最も受けやすい枠である。
現在入力に直接は出ていないが、意味的に近い過去話題や似た相談軸を、ベクトル hit から補助候補として戻してよい。
...
連想レーンは、既存 `commitment` の再順位付けには使ってよい。
ただし、vector hit だけで `active_commitments` の主候補を決めないほうがよい。
...
`episodic_evidence` は、先に選ばれた `memory_units` の根拠を補うのが基本である。
ただし、曖昧な回想や似た相談の連想では、連想レーンから直接候補を拾ってよい。

通常経路では、次の順序にする。

1. 選ばれた `memory_units.evidence_event_ids` に対応する `episode_digests` を引く
2. その中から今回の話題に最も近いものを絞る
3. なお不足する場合だけ、連想レーンの上位 `episode_digests` を補助候補として使う
...
1. `RecallHint` を生成する
2. 構造レーンで `active_commitments` と `relationship_model` を先に確認する
3. 構造レーンで `user_model` / `self_model` / `active_topics` の明示候補を集める
...
特に、MVP ではベクトル検索を同期想起の主導権にしないほうがよい。
まずは `scope` と `memory_type` に基づく構造レーンを軸にし、その補助として小さいベクトルレーンを使う。
...
- 過去回想が主題:
  - `episodic_evidence` と連想レーンの重みを上げる
- 約束確認が主題:
  - `active_commitments` を最優先に固定し、連想レーンは補助に留める
- 感情的な相談:
  - `relationship_model`、`user_model`、連想レーンの重みを上げる
