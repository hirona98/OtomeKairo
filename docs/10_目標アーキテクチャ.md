...
- `context assembler` は、`recent_dialog`、`selected_memory_pack`、`stable_self_state`、`confirmed_preferences`、`long_mood_state`、`reply_render_input`、`action_selection_context` を別断面で構成する
- `confirmed_preferences` と `long_mood_state` は、retrieval 用 `memory_snapshot` の副産物ではなく、`stable_preference_projection` と最新 `long_mood_state` から読む stable context として構成する
...
- `action_selection_context` と `reply_render_input` は、共通の `stable_preferences(likes/dislikes/revoked)` 断面を共有し、行動判断と返答生成で嗜好契約をずらさない
