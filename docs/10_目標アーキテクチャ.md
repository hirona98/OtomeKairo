...
- 現在の稼働面は、`browser_chat` を中心にした `テキスト入力`、ブラウザ録音からの `STT`、`camera_still_image` 添付、`camera_observation`、`browse` 完了後の `network_result` に限る
...
- current の `microphone` は常時センサーではなく、ブラウザ UI の録音操作を `POST /api/microphone/input` で `STT` に流す補助経路としてだけ接続している
