...
- 現在の人格ランタイムは `src/otomekairo/runtime/main_loop.py` の単一ループで、`settings_change_sets`、`settings_overrides`、`pending_inputs`、`browse task`、`memory_jobs` を順に処理する
- 現在の制御面 UI は `browser_chat` 専用で、`GET /api/status` は runtime 要約、感情要約、主注意の kind、task 件数だけを返す
- `microphone` の常時入力、`move`、`social`、`SNS` 入出力、`idle_tick` 起点の自発行動は target 側の語彙であり、current では未接続である
