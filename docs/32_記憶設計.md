...
- current 実装では、`event_entities` に `about_time` 由来の `about_year` と `life_stage` を追加してよい
...
- current 実装では、`preview_text` に event summary に加えて `source`、`kind`、`event_entities`、`event_threads`、`event_affect` の要約を含めてよい
