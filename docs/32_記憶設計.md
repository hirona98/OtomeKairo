...
- current 実装では、最終選別を完全な `LLM` 選別にせず、collector 群の候補収集と決定論的 selector を先に使い、その trace を `retrieval_runs.selected_json.selection_trace` に残してよい

<!-- Block: Final Selection -->
### 最終選別

- target では、候補の最終選別に `LLM` を使う構成を採ってよい
- current 実装では、`src/otomekairo/usecase/retrieval_plan.py` が `RetrievalPlan` を作り、`src/otomekairo/usecase/retrieval_collectors.py` が collector 群で候補を集め、`src/otomekairo/usecase/retrieval_selector.py` が決定論的 selector として `memory_bundle` を確定し、`src/otomekairo/usecase/retrieval_flow.py` が全体を束ねる
- current 実装では、候補の最終選別を追加の `LLM` 呼び出しへ渡さない
- current 実装の選別 trace は `retrieval_runs.selected_json.selection_trace` に保存し、collector ごとの採用傾向は `collector_counts` と `selector_summary` に残してよい
...
- 現在の実装では、`browse` 完了を含む周期から `summary` と外部確認済み `fact` を作り、`event_affect`、順序由来の `event_links`、短周期束ねの `event_threads`、`fact -> summary` の `state_links` まで保存する
...
- 初期に固定する主テーブルは、`input_journal`、`events`、`memory_states`、`preference_memory`、`event_affects`、`event_links`、`event_threads`、`event_about_time`、`state_links`、`event_entities`、`state_entities`、`revisions`、`retrieval_runs`、`event_preview_cache`、`memory_jobs`、`vec_items`、`events_fts` とする
...
- `event_about_time` は、`explicit_time` の構造探索に使う
...
- 長周期の適用で、`write_memory` は `memory_states`、`preference_memory`、`event_affects`、`event_links`、`event_threads`、`event_about_time`、`state_links`、`event_entities`、`revisions`、`vec_items` を更新し、`refresh_preview` だけが `event_preview_cache` を更新する
