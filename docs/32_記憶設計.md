...
- 初期実装では、`summary` / `fact` の候補収集に `sqlite-vec` の類似検索を使い、直近順だけに固定しない
- 初期実装では、完全な `RetrievalPlan` の前に、`current_observation` と `summary` / `fact` / 直近 `events` の決定論的な一致判定で候補を先に絞ってよい
...
- `relationship_hits` は、今回の判断で誰を重く扱うかを決める材料でもある
...
- 現在の実装では、`state_updates.upsert` に加えて `preference_updates`、`event_affect`、`context_updates` も同一 transaction で適用する
- 現在の実装では、`state_updates` の操作種別はまだ `upsert` のみを許可し、`close`、`mark_done`、`revise_confidence` は後続段階で実装する
...
- 初期実装では、`browse` の `network_result` を伴う周期からは、圧縮要約の `summary` だけでなく、外部確認済みの短い `fact` を別 `memory_state` として作ってよい
