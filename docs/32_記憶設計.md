...
- current 実装では、`selected_json.selector_summary.selector_mode=\"llm_ranked\"`、`selection_reason`、`selector_input_candidate_count`、`llm_selected_ref_count`、`slot_skipped_count` を追加し、`LLM` へ渡した候補量と返した順序と slot 上限適用結果を監査できるようにしてよい
