...
- current の記憶流入は、`chat_message`、`microphone_message`、`camera_observation`、`network_result`、`idle_tick` を前提にする
...
- 想起候補は `events.summary_text` や `memory_states.payload_json.summary_text` のような既存要約を使い、選別専用の派生キャッシュは持たない
...
- 更新で内容が変わる状態は、根拠イベントと変更理由を `memory_jobs` と適用結果から追えるようにする
- 同じ種類の情報でも、時期が違うなら上書きせず並存させる
- 記憶更新は、`self_state` の人格断面を直接変更せず、記憶・感情・嗜好・文脈の更新だけを扱う
...
- `domain` の初期固定値は、`action_type`、`observation_kind`、`topic_keyword` とする
...
- current 実装では、`chat_message` / `microphone_message` の明示表現から `topic_keyword` の `confirmed` を直接生成してよい
...
- `memory_bundle` は、`recent_event_window`、`working_memory_items`、`episodic_items`、`semantic_items`、`affective_items`、`relationship_items`、`preference_items`、`reflection_items` を持つ
...
- `relationship_items` は、`relation` の関連項目を持ち、今回の判断で誰を重く扱うかを決める材料である
- `preference_items` は、`stable_preference_projection` から再投影した `preference_memory` の関連項目を持ち、relation 系とは独立した slot budget で扱う
...
- current 実装では、`candidate_entries.text` は event summary や memory summary を優先して使い、候補本文の全量複製は行わない
...
- 記憶更新は、`enqueue -> claim -> dispatch(job_kind) -> apply -> followup enqueue -> embedding sync` の順で行う
...
flowchart TD
    ...
    apply --> followup["followup enqueue"]
...
- `MemoryWritePlan` は、少なくとも `event_annotations`、`state_updates`、`preference_updates`、`event_affect`、`context_updates` を持つ
...
- 現在の実装では、`browse` 完了を含む周期から `summary` と外部確認済み `fact` を作り、`event_affect`、`reply_to / continuation / same_topic / caused_by` を出し分けた `event_links`、短周期束ねと会話継続を併用した `event_threads`、`fact -> summary` の `state_links` まで保存する
...
- 初期に固定する主テーブルは、`input_journal`、`events`、`memory_states`、`preference_memory`、`event_affects`、`event_links`、`event_threads`、`event_about_time`、`state_about_time`、`state_links`、`event_entities`、`state_entities`、`memory_jobs`、`vec_items`、`events_fts` とする
...
- 短周期の保存では、`events`、`working_memory`、`recent_event_window`、新規に enqueue する `memory_jobs` の更新までを行う
...
- 長周期の適用で、`write_memory` は `memory_states`、`preference_memory`、`event_affects`、`event_links`、`event_threads`、`event_about_time`、`state_about_time`、`state_links`、`event_entities`、`vec_items` を更新し、必要なら `embedding_sync` を followup enqueue する
...
- 更新は、`memory_jobs` を起点に `dispatch(job_kind) -> 適用 -> followup enqueue -> embedding sync` で行う
...
- `memory_jobs` を持ち、記憶更新は長周期で再開可能にする
