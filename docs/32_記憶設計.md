...
- current の記憶処理が直接受ける入力は `chat_message`、`camera_observation`、`network_result` に限る
...
- `microphone`、`SNS`、`line_result`、`idle_tick` 起点の記憶流入は target 語彙としては残すが、current では未接続である
...
- target の `source` 語彙は、少なくとも `web_input`、`camera`、`microphone`、`network_result`、`sns_result`、`line_result`、`idle_tick`、`post_action_followup`、`self_initiated` を区別する
- current 実装で実際に出る `source` は、主に `web_input`、`self_initiated`、`network_result` である
...
- current 実装では、最終選別を完全な `LLM` 選別にせず、`sqlite-vec` の候補補強と決定論的 score を先に使い、その trace を `retrieval_runs.selected_json.selection_trace` に残してよい

<!-- Block: Final Selection -->
### 最終選別

- target では、候補の最終選別に `LLM` を使う構成を採ってよい
- current 実装では、`src/otomekairo/usecase/retrieval_flow.py` が `RetrievalPlan`、`sqlite-vec` の候補補強、決定論的 score を使って `memory_bundle` を直接組み立てる
- current 実装では、候補の最終選別を追加の `LLM` 呼び出しへ渡さない
- current 実装の選別 trace は `retrieval_runs.selected_json.selection_trace` に保存する
