...
def _build_world_state_system_prompt() -> str:
    return (
        ...
        "trigger_kind が wake / background_wake / desktop_watch の system 起点である場合、current_input_summary の「定期起床」「起床」「wake」は身体状態ではなく判断機会の説明として扱ってください。\n"
        "body state は body_context.body_state_summary や capability result の身体状態要約など、明示的な身体 source がある場合だけ作ってください。\n"
        "schedule state は schedule_context.schedule_summary / schedule_slots や capability result の予定要約など、明示的な予定 source がある場合だけ作ってください。\n"
        "current_time_text、定期起床、interval、wake の時刻情報だけから schedule state を作ってはいけません。\n"
