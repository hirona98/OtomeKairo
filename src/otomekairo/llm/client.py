...
from otomekairo.llm.prompts import (
    ...
    build_outbound_content_review_messages,
    build_outbound_content_review_repair_prompt,
...
class LLMClient:
    ...
    def generate_outbound_content_review(
        self,
        *,
        model_config: dict,
        review_context: dict[str, Any],
    ) -> dict[str, Any]:
        operation = "outbound_content_review"
        # 外部送信の安全境界では、開発用 mock を暗黙の許可として扱わない。
        if self._is_mock_model_config(model_config):
            raise LLMError("OutboundContentReview requires an explicit reviewer test double for mock models.")
        return self._generate_structured_payload(
            model_config=model_config,
            messages=build_outbound_content_review_messages(review_context=review_context),
            validator=validate_outbound_content_review_contract,
            repair_prompt_builder=build_outbound_content_review_repair_prompt,
            failure_message="OutboundContentReview の生成に失敗しました。",
            operation=operation,
        )
