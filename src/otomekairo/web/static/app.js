...
function renderConsoleClientSettings() {
  ...
  element("display-console-unavailable").hidden = available;
  element("motion-console-unavailable").hidden = available;
  if (!available) {
    renderAvatarPresentation();
    renderDisplaySettings(null);
    renderMotionSettings(null);
    return;
  }
  ...
  renderMotionSettings(settings.motion);
}

// CocoroConsole 端末の表示設定を読み取り専用で反映する。
function renderDisplaySettings(settings) {
  const display = settings?.display || {};
  const process = settings?.process || {};
  const messageWindow = display.message_window || {};
  element("display-client-id").value = state.consoleClient?.client_id || "";
  element("display-console-api-port").value = process.console_api_port ?? "";
  element("display-cocoro-shell-port").value = process.cocoro_shell_port ?? "";
  element("display-restore-window-position").checked = display.restore_window_position === true;
  element("display-topmost").checked = display.topmost === true;
  element("display-escape-cursor").checked = display.escape_cursor === true;
  element("display-avatar-position-x").value = display.avatar_position_x ?? "";
  element("display-avatar-position-y").value = display.avatar_position_y ?? "";
  element("display-touch-virtual-key-enabled").checked = display.touch_virtual_key_enabled === true;
  element("display-virtual-key").value = display.virtual_key || "";
  element("display-auto-move").checked = display.auto_move === true;
  element("display-avatar-window-size").value = display.avatar_window_size ?? "";
  element("display-ambient-occlusion").checked = display.ambient_occlusion_enabled === true;
  setSelectValue("display-msaa-level", display.msaa_level, "4");
  setSelectValue("display-avatar-shadow-mode", display.avatar_shadow_mode, "1");
  setSelectValue("display-avatar-shadow-resolution", display.avatar_shadow_resolution, "0");
  setSelectValue("display-background-shadow-mode", display.background_shadow_mode, "2");
  setSelectValue("display-background-shadow-resolution", display.background_shadow_resolution, "2");
  element("display-show-message-window").checked = display.show_message_window === true;
  element("display-message-max-count").value = messageWindow.max_message_count ?? "";
  element("display-message-max-chars").value = messageWindow.max_total_characters ?? "";
  element("display-message-min-size").value = messageWindow.min_window_size ?? "";
  element("display-message-max-size").value = messageWindow.max_window_size ?? "";
  element("display-message-font-size").value = messageWindow.font_size ?? "";
  element("display-message-horizontal-offset").value = messageWindow.horizontal_offset ?? "";
  element("display-message-vertical-offset").value = messageWindow.vertical_offset ?? "";
  renderEscapePositions(display.escape_positions || []);
  renderWindowPlacements(display.window_placements || {});
}

function setSelectValue(elementId, value, fallback) {
...
function renderEscapePositions(entries) {
  const container = element("display-escape-positions");
  if (!Array.isArray(entries) || !entries.length) {
    const empty = document.createElement("p");
    empty.className = "field-hint";
    empty.textContent = "移動先座標は未設定です。";
    container.replaceChildren(empty);
    return;
  }
  const rows = entries.map((entry, index) => {
    const row = document.createElement("div");
    row.className = "display-escape-position-row";
    const enabled = document.createElement("label");
    enabled.className = "checkbox-field";
    enabled.append(document.createTextNode(`#${index + 1}`));
    ...
    x.textContent = `X: ${entry?.x ?? "—"}`;
    const y = document.createElement("span");
    y.textContent = `Y: ${entry?.y ?? "—"}`;
    row.append(enabled, x, y);
    return row;
  });
  container.replaceChildren(...rows);
}

function renderWindowPlacements(placements) {
  const container = element("display-window-placements");
  const entries = Object.entries(placements || {});
  if (!entries.length) {
    const empty = document.createElement("p");
    empty.className = "field-hint";
    empty.textContent = "ウィンドウ配置は未設定です。";
    container.replaceChildren(empty);
    return;
  }
  const rows = entries.map(([key, placement]) => {
    const row = document.createElement("div");
...
// CocoroConsole 端末のモーション設定を読み取り専用で反映する。
function renderMotionSettings(motion) {
  const sets = Array.isArray(motion?.animation_sets) ? motion.animation_sets : [];
  const selectedId = motion?.selected_animation_set_id || sets[0]?.animation_set_id || "";
  setSelectOptions(element("motion-set-select"), sets, "animation_set_id", selectedId);
  const animationSet = arrayById(sets, "animation_set_id", selectedId) || null;
  element("motion-set-display-name").value = animationSet?.display_name || "";
  element("motion-set-id").value = animationSet?.animation_set_id || "";
  element("motion-loop-standing").value = animationSet?.posture_change_loop_count_standing ?? "";
  element("motion-loop-sitting").value = animationSet?.posture_change_loop_count_sitting_floor ?? "";
  renderMotionAnimations(animationSet?.animations || []);
}

function renderMotionAnimations(animations) {
  const container = element("motion-animation-list");
  ...
  for (const animation of animations) {
    const type = Number.isInteger(animation?.animation_type) ? animation.animation_type : -1;
    if (!groups.has(type)) {
      groups.set(type, []);
    }
  ...
  const sections = [...groups.entries()].map(([type, items]) => {
    ...
    for (const animation of items) {
      ...
      const name = animation.display_name || animation.animation_name || "animation";
      label.append(document.createTextNode(name));
      const input = document.createElement("input");
      input.type = "checkbox";
      input.checked = animation.enabled === true;
      input.disabled = true;
      label.append(input);
      list.append(label);
    }
