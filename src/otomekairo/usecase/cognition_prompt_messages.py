"""Prompt message builders for cognition and reply rendering."""
...
# Block: Cognition plan prompt build
def build_plan_messages(request: CognitionPlanRequest) -> list[dict[str, Any]]:
    ...
    system_prompt = "\n".join(
        [
            ...
            _persona_bias_prompt_line(title="感情補正", biases=persona_projection["emotion_bias"]),
            _persona_bias_prompt_line(title="内部欲求補正", biases=persona_projection["drive_bias"]),
...
# Block: Retrieval selection prompt build
def build_retrieval_selection_messages(
...
# Block: Reply render prompt build
def build_reply_render_messages(request: ReplyRenderRequest) -> list[dict[str, Any]]:
...
def _stable_confirmed_preferences_prompt_line(
    stable_confirmed_preferences: dict[str, Any],
) -> str:
...
def _task_entries_prompt_text(task_entries: list[dict[str, Any]]) -> str:
    ...
    for task_entry in task_entries[:3]:
        ...
        summaries.append(f"{task_kind}:{_memory_prompt_text(goal_hint)}({relative_time_text})")
    return " / ".join(summaries)
