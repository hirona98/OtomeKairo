...
- `src/otomekairo/infra/sqlite/backend.py`: `core_schema.sql` 適用判定、schema/meta 検証、singleton seed の入口、共通 connection を束ねる bootstrap facade であり、実処理は `bootstrap_connection_impl.py`、`bootstrap_meta_impl.py`、`bootstrap_settings_editor_impl.py`、`bootstrap_singleton_seed_impl.py` に分離している
- `src/otomekairo/infra/sqlite/`: `runtime_query_impl.py`、`cycle_commit_impl.py`、`settings_impl.py`、`runtime_live_state_impl.py`、`event_writer_impl.py`、`ui_event_impl.py`、`runtime_lease_impl.py`、`memory_job_impl.py` が責務別の SQLite 実装を持ち、`*_store.py` は gateway port を包む薄い adapter、`write_memory_execution_store.py` は `write_memory_load_impl.py`、`write_memory_state_impl.py`、`write_memory_preference_impl.py`、`write_memory_context_impl.py` を束ねる `write_memory` 専用 execution adapter であり、`write_memory_state_impl.py` は `write_memory_state_update_impl.py` / `write_memory_self_state_sync_impl.py`、`write_memory_context_impl.py` は `write_memory_event_affect_impl.py` / `write_memory_context_relation_impl.py` へさらに分けている
