...
この profile は通常会話 1 回、`external.status / schedule.status / device.status / body.status / environment.status / location.status` の capability request / result follow-up、memory postprocess drain を確認する。
