...
設定画面では、アバターごとの STT / TTS、通常の音声入力に使用するマイク、話者識別を編集できる。
...
生成系モデルは選択中の `model_preset` 1 件、埋め込みモデルは `memory_sets.embedding` で管理する。
通常はブラウザ UI または `CocoroConsole` の設定画面から編集する。

`model` が `mock` で始まるときは、内蔵の開発用モック経路を使う。
それ以外の生成処理は LiteLLM を通して呼び出す。
...
実 LLM 用の API key を `var/otomekairo/config.db` に設定している場合、隔離データディレクトリへ model / memory 設定だけをコピーして短い実 LLM smoke を実行する。
