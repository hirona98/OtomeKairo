...
この profile は隔離データディレクトリでサーバを起動し、通常会話、定期起床（`background_wake`）、capability request / result follow-up、memory worker、vision capture、記憶と想起の代表ケースを確認する。
...
この profile は通常会話、status capability の request / result follow-up、API起床（manual wake）の自律判断 matrix、`vision.capture` result follow-up、定期起床（`background_wake`）制御 matrix、memory postprocess drain を確認する。
