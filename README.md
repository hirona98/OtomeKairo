...
- `src/otomekairo/usecase/run_cognition.py`: 認知クライアントが返す `cognition_result` を受け取り、最小の `action_command` を使って `speak` は `token` / `message`、`notify` は `notice` として実行し、`action_history` へ変換する
- `src/otomekairo/usecase/validate_action.py`: `cognition_result.action_proposals` から `speak` / `browse` / `notify` / `wait` を比較し、`execute / hold / reject` と最小の `action_command` を確定する
