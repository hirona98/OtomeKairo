...
- `src/otomekairo/usecase/validate_action.py`: `cognition_result.action_proposals` から `speak` / `browse` / `notify` / `wait` を比較し、現実に実行できない候補は `hold` / `reject` として明示する最小の `action validator` を持つ
